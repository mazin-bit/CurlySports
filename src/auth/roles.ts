...
export function hasPermission(role: string | null, permission: string): boolean {
  ...
  return Array.isArray(list) && list.includes(permission as PermissionValue);
}
 ...
 */
export function isAtLeast(role: string | null, minimumRole: string): boolean {
  const order: string[] = [ROLES.MEMBER, ROLES.ADMIN, ROLES.SUPER_ADMIN];
...
/**
 * Can this role assign the target role?
 */
