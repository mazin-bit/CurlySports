...
import { getFootballStandingsFallback, getFootballSeasonYears, FOOTBALL_LEAGUES_WITH_HISTORY } from './data/footballStandingsFallback';
import { F1Section, useF1Data } from './f1/F1Section';
...
/** How often to refresh Top Players data from API (ms). Use 30000+ to avoid rate limits; UI still shows "Updated Xs ago" every second. */
const LIVE_PLAYERS_REFRESH_INTERVAL_MS = 15000; // 15s so goals/runs/wickets update as players score
...
const SPORTS_CONFIG = {
  soccer: {
    ...
    leagueLogos: {
      ...
      worldcup: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/2026_FIFA_World_Cup_emblem_%28without_trophy%29.svg/500px-2026_FIFA_World_Cup_emblem_%28without_trophy%29.svg.png'
    }
  },
  basketball: {
    label: 'Basketball',
  ...
  cricket: {
    ...
    leagueLogos: {
      ipl: 'https://wsrv.nl/?url=' + encodeURIComponent('https://upload.wikimedia.org/wikipedia/commons/thumb/5/52/Ipl.svg/200px-Ipl.svg.png') + '&w=200&h=200&fit=contain',
      bbl: 'https://wsrv.nl/?url=' + encodeURIComponent('https://a.espncdn.com/i/leaguelogos/cricket/500/8044.png') + '&w=200&h=200&fit=contain',
      psl: 'https://wsrv.nl/?url=' + encodeURIComponent('https://upload.wikimedia.org/wikipedia/commons/thumb/4/4d/Psl-icon.png/200px-Psl-icon.png') + '&w=200&h=200&fit=contain',
      ilt20: 'https://wsrv.nl/?url=' + encodeURIComponent('https://a.espncdn.com/i/leaguelogos/cricket/500/20921.png') + '&w=200&h=200&fit=contain',
      sa20: 'https://wsrv.nl/?url=' + encodeURIComponent('https://a.espncdn.com/i/leaguelogos/cricket/500/21275.png') + '&w=200&h=200&fit=contain',
      t20wc: 'https://wsrv.nl/?url=' + encodeURIComponent('https://upload.wikimedia.org/wikipedia/commons/thumb/8/8e/T20_WC_Logo.png/200px-T20_WC_Logo.png') + '&w=200&h=200&fit=contain',
      ranji: 'https://wsrv.nl/?url=' + encodeURIComponent('https://upload.wikimedia.org/wikipedia/en/thumb/2/23/Ranji_Trophy_logo.png/200px-Ranji_Trophy_logo.png') + '&w=200&h=200&fit=contain',
      sheffield: 'https://wsrv.nl/?url=' + encodeURIComponent('https://a.espncdn.com/i/leaguelogos/cricket/500/8043.png') + '&w=200&h=200&fit=contain',
      county: 'https://wsrv.nl/?url=' + encodeURIComponent('https://a.espncdn.com/i/leaguelogos/cricket/500/8049.png') + '&w=200&h=200&fit=contain',
      icc_test: 'https://wsrv.nl/?url=' + encodeURIComponent('https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/ICC_logo.png/200px-ICC_logo.png') + '&w=200&h=200&fit=contain'
    }
  },
  f1: {
    label: 'Formula 1',
...
/** FIFA World Cup knockout bracket placeholders: group/position labels (e.g. Group A 1st vs Group B 2nd) for each round. */
const WORLD_CUP_KNOCKOUT_BRACKET = [
  {
    round: 'Round of 32',
    matches: [
      { home: 'Group A 1st', away: 'Group B 2nd' },
      { home: 'Group B 1st', away: 'Group A 2nd' },
      { home: 'Group C 1st', away: 'Group D 2nd' },
      { home: 'Group D 1st', away: 'Group C 2nd' },
      { home: 'Group E 1st', away: 'Group F 2nd' },
      { home: 'Group F 1st', away: 'Group E 2nd' },
      { home: 'Group G 1st', away: 'Group H 2nd' },
      { home: 'Group H 1st', away: 'Group G 2nd' },
      { home: 'Group I 1st', away: 'Group J 2nd' },
      { home: 'Group J 1st', away: 'Group I 2nd' },
      { home: 'Group K 1st', away: 'Group L 2nd' },
      { home: 'Group L 1st', away: 'Group K 2nd' },
      { home: '3rd Group A/C/D', away: '3rd Group B/E/F' },
      { home: '3rd Group G/H/I', away: '3rd Group J/K/L' },
      { home: '3rd place (TBD)', away: '3rd place (TBD)' },
      { home: '3rd place (TBD)', away: '3rd place (TBD)' }
    ]
  },
  {
    round: 'Round of 16',
    matches: [
      { home: 'Winner (1A-2B)', away: 'Winner (1C-2D)' },
      { home: 'Winner (1E-2F)', away: 'Winner (1G-2H)' },
      { home: 'Winner (1I-2J)', away: 'Winner (1K-2L)' },
      { home: 'Winner (R32-13)', away: 'Winner (R32-14)' },
      { home: 'Winner (1B-2A)', away: 'Winner (1D-2C)' },
      { home: 'Winner (1F-2E)', away: 'Winner (1H-2G)' },
      { home: 'Winner (1J-2I)', away: 'Winner (1L-2K)' },
      { home: 'Winner (R32-15)', away: 'Winner (R32-16)' }
    ]
  },
  {
    round: 'Quarterfinal',
  ...
  },
  {
    round: 'Third-Place',
    matches: [
      { home: 'Semifinal loser 1', away: 'Semifinal loser 2' }
    ]
  },
  {
    round: 'Final',
...
// Reverse map: ESPN athlete id (string) -> our internal player id (for merging live stats)
const SOCCER_ESPN_ID_TO_OUR_ID = Object.fromEntries(
  Object.entries(SOCCER_ESPN_IDS).map(([ourId, espnId]) => [String(espnId), Number(ourId)])
);

// Soccer league codes used for fetching live goals/assists leaders (ESPN statistics API)
const SOCCER_STATS_LEAGUES = ['eng.1', 'esp.1', 'ger.1', 'ita.1', 'fra.1', 'uefa.champions', 'usa.1', 'ksa.1'];

// Career goals/assists baselines sourced from ESPN and official statistics (ESPN’s public API does not expose career totals; we add live season from ESPN so displayed total updates as they score).
const SOCCER_CAREER_BASELINE = {
  45843: { goals: 858, assists: 382 },   // Messi (before 2026 MLS)
  22712: { goals: 910, assists: 257 },    // Ronaldo (before 2025-26 Saudi)
  129596: { goals: 435, assists: 267 },   // Neymar
  210513: { goals: 248, assists: 60 },    // Haaland
  231388: { goals: 348, assists: 169 },   // Mbappé
  238861: { goals: 92, assists: 73 },    // Vinícius Júnior
  270438: { goals: 65, assists: 40 },    // Bellingham
  491564: { goals: 22, assists: 16 },     // Lamine Yamal
  126131: { goals: 612, assists: 139 },   // Lewandowski
  159665: { goals: 360, assists: 159 },   // Salah
  158500: { goals: 428, assists: 95 },    // Kane
  423549: { goals: 26, assists: 16 },    // Pedri
  139867: { goals: 165, assists: 280 },  // De Bruyne
  231908: { goals: 40, assists: 36 },     // Rodri
  227181: { goals: 65, assists: 50 },     // Odegaard
  140019: { goals: 262, assists: 118 },  // Griezmann
  391585: { goals: 128, assists: 45 },   // Lautaro Martínez
  391857: { goals: 98, assists: 22 },    // Osimhen
  444222: { goals: 42, assists: 52 },     // Wirtz
  410313: { goals: 52, assists: 45 },    // Musiala
  139871: { goals: 58, assists: 18 },    // van Dijk
  253919: { goals: 22, assists: 12 },     // Rice
  424911: { goals: 30, assists: 22 },     // Cole Palmer
  210411: { goals: 118, assists: 95 },    // Sané
  394392: { goals: 52, assists: 42 },    // Rafael Leão
  391157: { goals: 95, assists: 18 },    // Vlahović
  233777: { goals: 20, assists: 38 },    // Barella
  191695: { goals: 68, assists: 72 },    // Bernardo Silva
  233775: { goals: 12, assists: 8 },     // Rúben Dias
  238864: { goals: 65, assists: 42 },    // Phil Foden
  186001: { goals: 58, assists: 78 },    // Kimmich
  231909: { goals: 28, assists: 35 },    // Theo Hernández
  393952: { goals: 12, assists: 8 },     // Bastoni
  403164: { goals: 58, assists: 52 },     // Bukayo Saka
  423546: { goals: 4, assists: 2 },      // Saliba
  254246: { goals: 22, assists: 22 },    // Valverde
  161986: { goals: 18, assists: 6 },     // Rüdiger
  210521: { goals: 22, assists: 32 },     // Frenkie de Jong
  403254: { goals: 35, assists: 28 },    // Bruno Guimarães
  238863: { goals: 102, assists: 22 },    // Alexander Isak
  247291: { goals: 12, assists: 14 },    // Enzo Fernández
  391856: { goals: 87, assists: 62 },     // Nkunku
  233774: { goals: 62, assists: 38 },     // Chiesa
  254245: { goals: 112, assists: 42 },    // Diogo Jota
  394393: { goals: 12, assists: 22 },     // Alphonso Davies
  194321: { goals: 52, assists: 45 },    // Dani Olmo
  457806: { goals: 92, assists: 58 },     // Raphinha
  391854: { goals: 6, assists: 2 },      // Romero
};
const COMMONS_BASE = 'https://upload.wikimedia.org/wikipedia/commons/';
const WSRV = 'https://wsrv.nl/?url='; // image proxy with CORS so images actually load
const enc = (u) => encodeURIComponent(u);

const PLAYER_HEADSHOT_OVERRIDES = {
  45843: WSRV + enc(COMMONS_BASE + 'thumb/b/b4/Lionel-Messi-Argentina-2022-FIFA-World-Cup_(cropped).jpg/350px-Lionel-Messi-Argentina-2022-FIFA-World-Cup_(cropped).jpg') + '&w=350&h=350&fit=cover',
  22712: WSRV + enc(COMMONS_BASE + 'thumb/8/8c/Cristiano_Ronaldo_2018.jpg/350px-Cristiano_Ronaldo_2018.jpg') + '&w=350&h=350&fit=cover',
  129596: WSRV + enc(COMMONS_BASE + 'thumb/9/91/Neymar_visiting_Red_Bull_Arena_(cropped).jpg/350px-Neymar_visiting_Red_Bull_Arena_(cropped).jpg') + '&w=350&h=350&fit=cover',
  210513: WSRV + enc(COMMONS_BASE + 'thumb/8/85/Erling_Haaland_2023_(cropped_square).jpg/350px-Erling_Haaland_2023_(cropped_square).jpg') + '&w=350&h=350&fit=cover',
  231388: WSRV + enc(COMMONS_BASE + 'thumb/4/45/Kylian_Mbappé.jpg/350px-Kylian_Mbappé.jpg') + '&w=350&h=350&fit=cover',
  238861: WSRV + enc(COMMONS_BASE + 'thumb/3/37/Vinicius_Jr_2018_(cropped).jpg/350px-Vinicius_Jr_2018_(cropped).jpg') + '&w=350&h=350&fit=cover',
  270438: WSRV + enc(COMMONS_BASE + 'thumb/8/88/Jude_Bellingham_Laureus_2024_(cropped2).jpg/350px-Jude_Bellingham_Laureus_2024_(cropped2).jpg') + '&w=350&h=350&fit=cover',
  491564: WSRV + enc(COMMONS_BASE + 'thumb/2/2c/Lamine_Yamal_in_2025_(cropped).jpg/350px-Lamine_Yamal_in_2025_(cropped).jpg') + '&w=350&h=350&fit=cover',
  126131: WSRV + enc(COMMONS_BASE + 'thumb/3/39/Robert_Lewandowski_2018.jpg/350px-Robert_Lewandowski_2018.jpg') + '&w=350&h=350&fit=cover',
  159665: WSRV + enc(COMMONS_BASE + 'thumb/4/4a/Mohamed_Salah_2018.jpg/350px-Mohamed_Salah_2018.jpg') + '&w=350&h=350&fit=cover',
  158500: WSRV + enc(COMMONS_BASE + 'thumb/7/72/Harry_Kane_2022_World_Cup_(cropped).jpg/350px-Harry_Kane_2022_World_Cup_(cropped).jpg') + '&w=350&h=350&fit=cover',
  139867: WSRV + enc(COMMONS_BASE + 'thumb/3/36/Kevin_De_Bruyne.jpg/350px-Kevin_De_Bruyne.jpg') + '&w=350&h=350&fit=cover',
};

const getHeadshot = (id) => {
  if (PLAYER_HEADSHOT_OVERRIDES[id]) return PLAYER_HEADSHOT_OVERRIDES[id];
  // Every other soccer player: ESPN headshot via wsrv.nl so images load in browser
  const espnId = SOCCER_ESPN_IDS[id] || id;
  const espnUrl = `https://a.espncdn.com/combiner/i?img=/i/headshots/soccer/players/full/${espnId}.png&w=350&h=254`;
  return WSRV + enc(espnUrl) + '&w=350&h=350&fit=cover';
};
...
// Map our cricket player IDs to correct ESPNcricinfo headshot IDs (when they differ)
const CRICKET_ESPN_HEADSHOT_IDS = {
  10617: 28081,    // MS Dhoni (our id was wrong – 10617 is Stuart Broad on ESPN)
  28081: 489889,   // Pat Cummins (our id 28081 is Dhoni on ESPN; Cummins is 489889)
  56144: 536936,   // Litton Das
  56145: 538506,   // Taskin Ahmed
  56146: 330902,   // Mustafizur Rahman
  56147: 56029,    // Mushfiqur Rahim
};
// Overrides: full image URL when ESPN has no/wrong headshot (Wikimedia Commons via wsrv.nl)
const CRICKET_HEADSHOT_OVERRIDES = {
  341016: WSRV + enc(COMMONS_BASE + 'thumb/f/f4/Ravichandran_Ashwin_%282%29.jpg/350px-Ravichandran_Ashwin_%282%29.jpg') + '&w=350&h=350&fit=cover',  // Ravichandran Ashwin
  447261: WSRV + enc(COMMONS_BASE + '5/5e/Kuldeep_Yadav.jpg') + '&w=350&h=350&fit=cover',  // Kuldeep Yadav
  931581: WSRV + enc(COMMONS_BASE + 'thumb/0/02/Rishabh_Pant_%2829693622367%29_%28cropped%29.jpg/350px-Rishabh_Pant_%2829693622367%29_%28cropped%29.jpg') + '&w=350&h=350&fit=cover',  // Rishabh Pant
  481235: WSRV + enc(COMMONS_BASE + 'thumb/9/9b/Mohammed_Shami_%28Indian_Cricket_team_training_SCG_2015%29.jpg/350px-Mohammed_Shami_%28Indian_Cricket_team_training_SCG_2015%29.jpg') + '&w=350&h=350&fit=cover',  // Mohammed Shami
  56143: WSRV + enc(COMMONS_BASE + 'thumb/c/c0/Shakib_Al_Hasan_%284%29_%28cropped%29.jpg/350px-Shakib_Al_Hasan_%284%29_%28cropped%29.jpg') + '&w=350&h=350&fit=cover',  // Shakib Al Hasan
  56147: WSRV + enc(COMMONS_BASE + 'thumb/e/e4/Mushfiqur_Rahim_2018_%28cropped%29.jpg/350px-Mushfiqur_Rahim_2018_%28cropped%29.jpg') + '&w=350&h=350&fit=cover',  // Mushfiqur Rahim
  // More players so ~23 missing get images (Commons via wsrv.nl)
  253802: WSRV + enc(COMMONS_BASE + 'thumb/1/15/Virat_Kohli_portrait.jpg/350px-Virat_Kohli_portrait.jpg') + '&w=350&h=350&fit=cover',  // Virat Kohli
  34102: WSRV + enc(COMMONS_BASE + 'thumb/2/20/Rohit_Sharma_November_2016.jpg/350px-Rohit_Sharma_November_2016.jpg') + '&w=350&h=350&fit=cover',  // Rohit Sharma
  28081: WSRV + enc(COMMONS_BASE + 'thumb/6/69/Pat_Cummins_fielding_Ashes_2021_%28cropped%29.jpg/350px-Pat_Cummins_fielding_Ashes_2021_%28cropped%29.jpg') + '&w=350&h=350&fit=cover',  // Pat Cummins
  1175352: WSRV + enc(COMMONS_BASE + '3/30/Shaheen_Afridi.jpg') + '&w=350&h=350&fit=cover',  // Shaheen Afridi
  230853: WSRV + enc(COMMONS_BASE + '8/8b/Ben_Stokes.jpg') + '&w=350&h=350&fit=cover',  // Ben Stokes
  348144: WSRV + enc(COMMONS_BASE + '1/1f/Babar_Azam.png') + '&w=350&h=350&fit=cover',  // Babar Azam
  232364: WSRV + enc(COMMONS_BASE + 'thumb/c/c0/Joe_root.jpg/350px-Joe_root.jpg') + '&w=350&h=350&fit=cover',  // Joe Root
  625383: WSRV + enc(COMMONS_BASE + '1/1f/Jasprit_Bumrah_%282%29.jpg') + '&w=350&h=350&fit=cover',  // Jasprit Bumrah
  311592: WSRV + enc(COMMONS_BASE + 'c/cb/Mitchell_Starc_2.jpg') + '&w=350&h=350&fit=cover',  // Mitchell Starc
  308967: WSRV + enc(COMMONS_BASE + 'thumb/7/7f/Jos_buttler.JPG/350px-Jos_buttler.JPG') + '&w=350&h=350&fit=cover',  // Jos Buttler
  529963: WSRV + enc(COMMONS_BASE + 'thumb/c/c3/Shai_Hope.jpg/350px-Shai_Hope.jpg') + '&w=350&h=350&fit=cover',  // Shai Hope
};

/** Cricket player ID -> exact Wikipedia page title for image fallback (when primary image fails). */
const CRICKET_WIKIPEDIA_FALLBACK_NAMES = {
  422108: 'K. L. Rahul',  // KL Rahul
  10617: 'Mahendra Singh Dhoni',  // MS Dhoni
  267192: 'Steve Smith (cricketer)',  // disambiguation
  419232: 'Imam-ul-Haq',  // hyphen
  436757: 'Fakhar Zaman',
  529963: 'Shai Hope',  // exact title has thumbnail; "(cricketer)" 404s
  529964: 'Nicholas Pooran',
  529965: 'Shamar Joseph',
  529966: 'Jason Holder (cricketer)',
  276298: 'Andre Russell',
  529968: 'Rovman Powell',
  529969: 'Alzarri Joseph',
  326016: 'Rashid Khan (cricketer)',
  25913: 'Mohammad Nabi',
  497121: 'Wanindu Hasaranga',
  497124: 'Dasun Shanaka',
  326015: 'Nathan Lyon (cricketer)',
  277916: 'Kane Williamson',
  277912: 'Trent Boult',
  379143: 'Quinton de Kock',
  303669: 'Kagiso Rabada',
  46248: 'David Warner (cricketer)',
  896485: 'Mohammad Rizwan',
  960491: 'Shadab Khan (cricketer)',
  56144: 'Litton Das',
  56145: 'Taskin Ahmed',
  56146: 'Mustafizur Rahman (cricketer)',
  // NZ / SA / SL / Afg – use exact Wikipedia title so summary returns thumbnail when available
  518100: 'Mitchell Santner',
  277918: 'Devon Conway',
  518097: 'Daryl Mitchell (cricketer)',
  518099: 'Glenn Phillips (cricketer)',
  518098: 'Matt Henry (cricketer)',
  959767: 'Rachin Ravindra',
  481897: 'Heinrich Klaasen',
  481234: 'Anrich Nortje',
  497122: 'Maheesh Theekshana',
  497123: 'Pathum Nissanka',
  497125: 'Charith Asalanka',
  1243995: 'Rahmanullah Gurbaz',
};

const getCricketHeadshot = (id) => {
  if (CRICKET_HEADSHOT_OVERRIDES[id]) return CRICKET_HEADSHOT_OVERRIDES[id];
  const espnId = CRICKET_ESPN_HEADSHOT_IDS[id] || id;
  const espnUrl = `https://a.espncdn.com/combiner/i?img=/i/headshots/cricket/players/full/${espnId}.png&w=350&h=254`;
  return WSRV + enc(espnUrl) + '&w=350&h=350&fit=cover';
};
...
// Career stats, trophies, achievements: sourced from ESPN and official statistics. Live goals/assists refresh from ESPN every 15s.
const PLAYERS_DATA = [
  // --- THE REAL STARS (NO DUPLICATES) ---
  { id: 45843, name: 'Lionel Messi', club: 'Inter Miami', position: 'forwards', rating: 94, goals: 865, assists: 385, image: getHeadshot(45843), age: 37, height: '1.70m', weight: '72kg',
    trophies: ['8× Ballon d\'Or', 'FIFA World Cup 2022', 'Copa América 2021', '4× UEFA Champions League', '10× La Liga', '2× Ligue 1', 'MLS Cup 2023', 'Leagues Cup 2023', 'Copa del Rey (7×)', 'UEFA Super Cup', 'FIFA Club World Cup (3×)', 'CONMEBOL–UEFA Cup of Champions 2022'],
    career: ['Barcelona', 'PSG', 'Inter Miami'],
    careerStations: [{ team: 'Barcelona', period: '2004–2021', goals: 672, assists: 303, apps: 778 }, { team: 'Paris Saint-Germain', period: '2021–2023', goals: 32, assists: 35, apps: 75 }, { team: 'Inter Miami', period: '2023–', goals: 35, assists: 28, apps: 45 }, { team: 'Argentina', period: '2005–', goals: 108, assists: 56, apps: 186 }],
    statsByCompetition: { 'La Liga': { goals: 474, assists: 192 }, 'Champions League': { goals: 129, assists: 42 }, 'Copa del Rey': { goals: 56, assists: 37 }, 'Ligue 1': { goals: 22, assists: 31 }, 'MLS': { goals: 28, assists: 24 }, 'International': { goals: 108, assists: 56 } },
    achievements: ['Most goals in a calendar year (91)', 'Top scorer in La Liga history', 'Argentina all-time top scorer', 'Most Ballon d\'Or wins (8)', 'Most goals for a single club (Barcelona)', 'UEFA Champions League top scorer (6×)'] },
  { id: 22712, name: 'Cristiano Ronaldo', club: 'Al Nassr', position: 'forwards', rating: 94, goals: 935, assists: 260, image: getHeadshot(22712), age: 39, height: '1.87m', weight: '83kg',
    trophies: ['5× Ballon d\'Or', 'UEFA Euro 2016', '5× UEFA Champions League', '3× Premier League', '2× La Liga', '2× Serie A', '2× FIFA Club World Cup', 'UEFA Nations League 2019', 'Arab Club Champions Cup 2023', 'Saudi Super Cup', 'Saudi Pro League 2023–24'],
    career: ['Sporting CP', 'Man Utd', 'Real Madrid', 'Juventus', 'Al Nassr'],
    careerStations: [{ team: 'Sporting CP', period: '2002–2003', goals: 5, assists: 6, apps: 31 }, { team: 'Manchester United', period: '2003–2009', goals: 118, assists: 69, apps: 292 }, { team: 'Real Madrid', period: '2009–2018', goals: 450, assists: 131, apps: 438 }, { team: 'Juventus', period: '2018–2021', goals: 101, assists: 22, apps: 134 }, { team: 'Manchester United', period: '2021–2022', goals: 27, assists: 5, apps: 54 }, { team: 'Al Nassr', period: '2023–', goals: 64, assists: 18, apps: 72 }, { team: 'Portugal', period: '2003–', goals: 130, assists: 44, apps: 212 }],
    statsByCompetition: { 'Premier League': { goals: 103, assists: 37 }, 'La Liga': { goals: 311, assists: 95 }, 'Serie A': { goals: 81, assists: 19 }, 'Champions League': { goals: 140, assists: 42 }, 'Saudi Pro League': { goals: 58, assists: 18 }, 'International': { goals: 130, assists: 44 } },
    achievements: ['All-time top scorer in football history', 'UEFA Champions League all-time top scorer', 'Portugal all-time top scorer', 'Most international goals (130)', 'Only player to score in 5 World Cups', 'Real Madrid all-time top scorer'] },
  { id: 129596, name: 'Neymar Jr', club: 'Al Hilal', position: 'forwards', rating: 91, goals: 437, assists: 269, image: getHeadshot(129596), age: 32, height: '1.75m', weight: '68kg',
    trophies: ['1× UEFA Champions League', '1× Copa Libertadores', '2× La Liga', '5× Ligue 1', 'Copa del Rey', 'Olympic Silver 2012'],
    career: ['Santos', 'Barcelona', 'PSG', 'Al Hilal'],
    careerStations: [{ team: 'Santos', period: '2009–2013', goals: 136, assists: 35, apps: 225 }, { team: 'Barcelona', period: '2013–2017', goals: 105, assists: 76, apps: 186 }, { team: 'Paris Saint-Germain', period: '2017–2023', goals: 118, assists: 77, apps: 173 }, { team: 'Al Hilal', period: '2023–', goals: 8, assists: 12, apps: 22 }, { team: 'Brazil', period: '2010–', goals: 79, assists: 56, apps: 128 }],
    statsByCompetition: { 'La Liga': { goals: 68, assists: 52 }, 'Ligue 1': { goals: 82, assists: 51 }, 'Champions League': { goals: 43, assists: 33 }, 'Saudi Pro League': { goals: 8, assists: 12 }, 'International': { goals: 79, assists: 56 } },
    achievements: ['Brazil all-time top scorer', 'Santos youngest goalscorer', 'Olympic silver 2012', 'Copa Libertadores winner'] },
  { id: 210513, name: 'Erling Haaland', club: 'Man City', position: 'forwards', rating: 91, goals: 252, assists: 62, image: getHeadshot(210513), age: 24, height: '1.94m', weight: '88kg',
    trophies: ['1× UEFA Champions League', '2× Premier League (2022–23, 2023–24)', '1× Bundesliga', '2× Austrian Bundesliga', 'UEFA Champions League top scorer 2022–23', 'Premier League Golden Boot (2×)', 'FA Cup 2023–24'],
    career: ['Molde', 'RB Salzburg', 'Dortmund', 'Man City'],
    careerStations: [{ team: 'Molde', period: '2017–2019', goals: 20, assists: 6, apps: 50 }, { team: 'RB Salzburg', period: '2019–2020', goals: 29, assists: 7, apps: 27 }, { team: 'Borussia Dortmund', period: '2020–2022', goals: 86, assists: 23, apps: 89 }, { team: 'Man City', period: '2022–', goals: 90, assists: 18, apps: 96 }, { team: 'Norway', period: '2019–', goals: 27, assists: 8, apps: 35 }],
    statsByCompetition: { 'Premier League': { goals: 63, assists: 14 }, 'Bundesliga': { goals: 62, assists: 19 }, 'Champions League': { goals: 41, assists: 8 }, 'International': { goals: 27, assists: 8 } },
    achievements: ['Most goals in a single Premier League season (36)', 'Fastest to 50 Premier League goals', 'Champions League top scorer 2022–23'] },
  { id: 231388, name: 'Kylian Mbappé', club: 'Real Madrid', position: 'forwards', rating: 92, goals: 352, assists: 171, image: getHeadshot(231388), age: 25, height: '1.78m', weight: '73kg',
    trophies: ['1× FIFA World Cup', '1× UEFA Nations League', '7× Ligue 1', 'Coupe de France (4×)', 'Trophée des Champions', 'UEFA Champions League runner-up', 'Real Madrid 2024–25'],
    career: ['Monaco', 'PSG', 'Real Madrid'],
    careerStations: [{ team: 'Monaco', period: '2015–2018', goals: 27, assists: 16, apps: 60 }, { team: 'Paris Saint-Germain', period: '2017–2024', goals: 256, assists: 108, apps: 308 }, { team: 'Real Madrid', period: '2024–', goals: 12, assists: 8, apps: 18 }, { team: 'France', period: '2017–', goals: 57, assists: 32, apps: 82 }],
    statsByCompetition: { 'Ligue 1': { goals: 175, assists: 74 }, 'Champions League': { goals: 48, assists: 32 }, 'La Liga': { goals: 12, assists: 8 }, 'International': { goals: 57, assists: 32 } },
    achievements: ['PSG all-time top scorer', 'World Cup 2018 winner', 'World Cup 2022 Golden Boot', 'Youngest French WC scorer'] },
  { id: 238861, name: 'Vinícius Júnior', club: 'Real Madrid', position: 'forwards', rating: 91, goals: 95, assists: 75, image: getHeadshot(238861), age: 24, height: '1.76m', weight: '73kg',
    trophies: ['2× UEFA Champions League', '3× La Liga', '2× Copa del Rey', 'FIFA Club World Cup', 'UEFA Super Cup', 'La Liga 2023–24'],
    career: ['Flamengo', 'Real Madrid'],
    careerStations: [{ team: 'Flamengo', period: '2017–2018', goals: 14, assists: 3, apps: 37 }, { team: 'Real Madrid', period: '2018–', goals: 78, assists: 68, apps: 245 }, { team: 'Brazil', period: '2019–', goals: 3, assists: 4, apps: 35 }],
    statsByCompetition: { 'La Liga': { goals: 58, assists: 48 }, 'Champions League': { goals: 22, assists: 18 }, 'Copa del Rey': { goals: 5, assists: 4 }, 'International': { goals: 3, assists: 4 } },
    achievements: ['UCL Final goal scorer 2022', 'UCL Young Player of the Season', 'La Liga breakthrough star'] },
  { id: 270438, name: 'Jude Bellingham', club: 'Real Madrid', position: 'midfielders', rating: 90, goals: 68, assists: 42, image: getHeadshot(270438), age: 21, height: '1.86m', weight: '75kg',
    trophies: ['1× UEFA Champions League', '1× La Liga', '1× Copa del Rey', 'DFB-Pokal', 'Golden Boy 2023', 'UEFA Champions League 2023–24', 'La Liga 2023–24'],
    career: ['Birmingham', 'Dortmund', 'Real Madrid'],
    careerStations: [{ team: 'Birmingham City', period: '2019–2020', goals: 4, assists: 2, apps: 44 }, { team: 'Borussia Dortmund', period: '2020–2023', goals: 24, assists: 25, apps: 132 }, { team: 'Real Madrid', period: '2023–', goals: 23, assists: 12, apps: 52 }, { team: 'England', period: '2020–', goals: 5, assists: 3, apps: 35 }],
    statsByCompetition: { 'Bundesliga': { goals: 12, assists: 17 }, 'La Liga': { goals: 19, assists: 10 }, 'Champions League': { goals: 10, assists: 6 }, 'International': { goals: 5, assists: 3 } },
    achievements: ['Golden Boy 2023', 'Youngest Englishman to play at World Cup', 'La Liga top scorer (midfielder) 2023–24'] },
  { id: 491564, name: 'Lamine Yamal', club: 'FC Barcelona', position: 'forwards', rating: 85, goals: 24, assists: 18, image: getHeadshot(491564), age: 17, height: '1.78m', weight: '66kg',
    trophies: ['1× UEFA European Championship 2024', '1× La Liga 2023–24', 'UEFA Euro 2024 Young Player of the Tournament', 'Supercopa de España'],
    career: ['Barcelona'],
    careerStations: [{ team: 'Barcelona', period: '2023–', goals: 18, assists: 15, apps: 62 }, { team: 'Spain', period: '2024–', goals: 2, assists: 3, apps: 15 }],
    statsByCompetition: { 'La Liga': { goals: 12, assists: 10 }, 'Champions League': { goals: 3, assists: 3 }, 'International': { goals: 2, assists: 3 } },
    achievements: ['Youngest scorer in Euro history', 'Youngest La Liga goalscorer for Barça', 'Euro 2024 winner'] },
  { id: 126131, name: 'Robert Lewandowski', club: 'FC Barcelona', position: 'forwards', rating: 89, goals: 615, assists: 141, image: getHeadshot(126131), age: 36, height: '1.85m', weight: '81kg',
    trophies: ['1× UEFA Champions League', '10× Bundesliga', '1× La Liga', '7× Polish Footballer of the Year', 'UEFA Champions League top scorer', 'Bundesliga top scorer (7×)', 'DFB-Pokal', 'Supercopa de España'],
    career: ['Lech Poznan', 'Dortmund', 'Bayern', 'Barcelona'],
    careerStations: [{ team: 'Lech Poznan', period: '2008–2010', goals: 41, assists: 18, apps: 82 }, { team: 'Borussia Dortmund', period: '2010–2014', goals: 103, assists: 26, apps: 187 }, { team: 'Bayern Munich', period: '2014–2022', goals: 344, assists: 72, apps: 375 }, { team: 'Barcelona', period: '2022–', goals: 92, assists: 22, apps: 98 }, { team: 'Poland', period: '2008–', goals: 82, assists: 22, apps: 149 }],
    statsByCompetition: { 'Bundesliga': { goals: 312, assists: 65 }, 'La Liga': { goals: 59, assists: 16 }, 'Champions League': { goals: 92, assists: 25 }, 'International': { goals: 82, assists: 22 } },
    achievements: ['Most goals in a single Bundesliga season (41)', 'Bundesliga all-time second top scorer', 'Only player to win European Golden Shoe in two leagues'] },
  { id: 159665, name: 'Mohamed Salah', club: 'Liverpool', position: 'forwards', rating: 90, goals: 363, assists: 161, image: getHeadshot(159665), age: 32, height: '1.75m', weight: '71kg',
    trophies: ['1× UEFA Champions League', '1× Premier League', '2× FA Cup', '2× League Cup', 'FIFA Club World Cup', 'Africa Cup of Nations 2021', 'PFA Player of the Year (2×)', 'Premier League Golden Boot (3×)', 'UEFA Champions League 2018–19'],
    career: ['Basel', 'Chelsea', 'Roma', 'Liverpool'],
    careerStations: [{ team: 'Basel', period: '2012–2014', goals: 20, assists: 17, apps: 79 }, { team: 'Chelsea', period: '2014–2016', goals: 2, assists: 4, apps: 19 }, { team: 'Roma', period: '2015–2017', goals: 34, assists: 24, apps: 83 }, { team: 'Liverpool', period: '2017–', goals: 262, assists: 112, apps: 334 }, { team: 'Egypt', period: '2011–', goals: 58, assists: 28, apps: 98 }],
    statsByCompetition: { 'Premier League': { goals: 159, assists: 74 }, 'Champions League': { goals: 45, assists: 18 }, 'Serie A': { goals: 29, assists: 21 }, 'International': { goals: 58, assists: 28 } },
    achievements: ['Liverpool all-time Premier League top scorer', 'Most goals in a 38-game PL season (32)', 'Egypt all-time top scorer'] },
  { id: 158500, name: 'Harry Kane', club: 'Bayern Munich', position: 'forwards', rating: 91, goals: 432, assists: 97, image: getHeadshot(158500), age: 31, height: '1.88m', weight: '86kg',
    trophies: ['3× Premier League Golden Boot', '1× Bundesliga 2023–24', '1× FIFA World Cup Golden Boot 2018', 'UEFA Euro 2020 runner-up', 'DFB-Pokal', 'Bayern top scorer 2023–24'],
    career: ['Tottenham', 'Bayern Munich'],
    careerStations: [{ team: 'Tottenham', period: '2009–2023', goals: 280, assists: 58, apps: 435 }, { team: 'Bayern Munich', period: '2023–', goals: 44, assists: 12, apps: 45 }, { team: 'England', period: '2015–', goals: 65, assists: 19, apps: 97 }],
    statsByCompetition: { 'Premier League': { goals: 213, assists: 46 }, 'Bundesliga': { goals: 36, assists: 10 }, 'Champions League': { goals: 21, assists: 5 }, 'International': { goals: 65, assists: 19 } },
    achievements: ['England all-time top scorer', 'Tottenham all-time top scorer', 'Premier League 200-goal club'] },
  { id: 423549, name: 'Pedri', club: 'FC Barcelona', position: 'midfielders', rating: 88, goals: 28, assists: 18, image: getHeadshot(423549), age: 21, height: '1.74m', weight: '60kg',
    trophies: ['1× La Liga', '1× UEFA Euro 2020', '1× Copa del Rey', 'Supercopa de España', 'Golden Boy 2021'],
    career: ['Las Palmas', 'Barcelona'],
    careerStations: [{ team: 'Las Palmas', period: '2019–2020', goals: 4, assists: 6, apps: 36 }, { team: 'Barcelona', period: '2020–', goals: 22, assists: 12, apps: 135 }, { team: 'Spain', period: '2021–', goals: 2, assists: 2, apps: 24 }],
    statsByCompetition: { 'La Liga': { goals: 15, assists: 9 }, 'Champions League': { goals: 3, assists: 2 }, 'Copa del Rey': { goals: 2, assists: 1 }, 'International': { goals: 2, assists: 2 } },
    achievements: ['Golden Boy Award 2021', 'Youngest Spain scorer at Euro', 'La Masia graduate'] },
  { id: 139867, name: 'Kevin De Bruyne', club: 'Man City', position: 'midfielders', rating: 90, goals: 168, assists: 283, image: getHeadshot(139867), age: 33, height: '1.81m', weight: '70kg',
    trophies: ['1× UEFA Champions League', '6× Premier League', '2× FA Cup', '5× League Cup', 'FIFA Club World Cup', 'UEFA Super Cup', 'PL Playmaker of the Season (4×)'],
    career: ['Genk', 'Chelsea', 'Wolfsburg', 'Man City'],
    careerStations: [{ team: 'Genk', period: '2008–2012', goals: 16, assists: 25, apps: 97 }, { team: 'Chelsea', period: '2012–2014', goals: 9, assists: 7, apps: 9 }, { team: 'Wolfsburg', period: '2014–2015', goals: 16, assists: 27, apps: 73 }, { team: 'Man City', period: '2015–', goals: 102, assists: 218, apps: 382 }, { team: 'Belgium', period: '2010–', goals: 25, assists: 48, apps: 102 }],
    statsByCompetition: { 'Premier League': { goals: 68, assists: 112 }, 'Champions League': { goals: 14, assists: 22 }, 'Bundesliga': { goals: 10, assists: 21 }, 'International': { goals: 25, assists: 48 } },
    achievements: ['Most assists in a PL season (20)', 'PFA Player of the Year', 'Elite playmaker'] },
  { id: 231908, name: 'Rodri', club: 'Man City', position: 'midfielders', rating: 91, goals: 42, assists: 38, image: getHeadshot(231908), age: 28, height: '1.91m', weight: '82kg',
    trophies: ['1× Ballon d\'Or 2024', '1× UEFA Champions League', '4× Premier League', '1× UEFA Euro 2024', '2× FA Cup', 'FIFA Club World Cup', 'UEFA Nations League 2023'],
    career: ['Villarreal', 'Atletico', 'Man City'],
    careerStations: [{ team: 'Villarreal', period: '2017–2018', goals: 1, assists: 1, apps: 38 }, { team: 'Atletico Madrid', period: '2018–2019', goals: 3, assists: 2, apps: 47 }, { team: 'Man City', period: '2019–', goals: 35, assists: 32, apps: 248 }, { team: 'Spain', period: '2018–', goals: 3, assists: 3, apps: 58 }],
    statsByCompetition: { 'Premier League': { goals: 22, assists: 24 }, 'Champions League': { goals: 6, assists: 5 }, 'La Liga': { goals: 2, assists: 2 }, 'International': { goals: 3, assists: 3 } },
    achievements: ['UCL Final MOTM 2023', 'Euro 2024 winner', 'Ballon d\'Or 2024'] },
  { id: 227181, name: 'Martin Odegaard', club: 'Arsenal', position: 'midfielders', rating: 88, goals: 68, assists: 52, image: getHeadshot(227181), age: 25, height: '1.78m', weight: '68kg',
    trophies: ['1× La Liga', '1× Copa del Rey', 'Arsenal Player of the Season (2×)', 'Norwegian Footballer of the Year (4×)'],
    career: ['Stromsgodset', 'Real Madrid', 'Sociedad', 'Arsenal'],
    careerStations: [{ team: 'Stromsgodset', period: '2014–2015', goals: 5, assists: 7, apps: 25 }, { team: 'Real Madrid', period: '2015–2021', goals: 8, assists: 9, apps: 11 }, { team: 'Real Sociedad', period: '2019–2020', goals: 7, assists: 9, apps: 36 }, { team: 'Arsenal', period: '2021–', goals: 45, assists: 25, apps: 155 }, { team: 'Norway', period: '2014–', goals: 3, assists: 2, apps: 58 }],
    statsByCompetition: { 'Premier League': { goals: 38, assists: 22 }, 'La Liga': { goals: 10, assists: 12 }, 'Champions League': { goals: 5, assists: 4 }, 'International': { goals: 3, assists: 2 } },
    achievements: ['Youngest Norway debutant', 'Arsenal captain', 'Elite No.10'] },
  { id: 140019, name: 'Antoine Griezmann', club: 'Atletico', position: 'forwards', rating: 88, goals: 19, image: getHeadshot(140019), age: 33, height: '1.76m', weight: '73kg',
    trophies: ['1× FIFA World Cup 2018', '1× UEFA Europa League', '1× UEFA Super Cup', 'UEFA Euro 2016 runner-up', 'Atletico all-time top scorer'],
    career: ['Sociedad', 'Atletico', 'Barcelona', 'Atletico'],
    careerStations: [{ team: 'Real Sociedad', period: '2009–2014', goals: 52, assists: 18, apps: 202 }, { team: 'Atletico Madrid', period: '2014–2019', goals: 133, assists: 50, apps: 257 }, { team: 'Barcelona', period: '2019–2021', goals: 35, assists: 17, apps: 102 }, { team: 'Atletico Madrid', period: '2021–', goals: 78, assists: 42, apps: 165 }, { team: 'France', period: '2014–', goals: 44, assists: 38, apps: 132 }],
    statsByCompetition: { 'La Liga': { goals: 178, assists: 72 }, 'Champions League': { goals: 32, assists: 18 }, 'International': { goals: 44, assists: 38 } },
    achievements: ['Atletico all-time top scorer', 'World Cup 2018', 'Euro 2016 Golden Boot'] },
  { id: 391585, name: 'Lautaro Martínez', club: 'Inter', position: 'forwards', rating: 89, goals: 26, image: getHeadshot(391585), age: 27, height: '1.74m', weight: '72kg',
    trophies: ['1× FIFA World Cup 2022', '2× Serie A', '2× Copa América', '2× Coppa Italia', 'UEFA Champions League runner-up 2023', 'Serie A MVP 2023–24'],
    career: ['Racing Club', 'Inter Milan'],
    careerStations: [{ team: 'Racing Club', period: '2015–2018', goals: 27, assists: 6, apps: 60 }, { team: 'Inter Milan', period: '2018–', goals: 134, assists: 36, apps: 268 }, { team: 'Argentina', period: '2018–', goals: 29, assists: 8, apps: 62 }],
    statsByCompetition: { 'Serie A': { goals: 108, assists: 28 }, 'Champions League': { goals: 14, assists: 5 }, 'Copa América': { goals: 6, assists: 2 }, 'International': { goals: 29, assists: 8 } },
    achievements: ['Serie A MVP 2024', 'World Cup 2022 winner', 'Inter captain'] },
  { id: 391857, name: 'Victor Osimhen', club: 'Galatasaray', position: 'forwards', rating: 88, goals: 22, image: getHeadshot(391857), age: 25, height: '1.85m', weight: '78kg',
    trophies: ['1× Serie A 2022–23', 'African Footballer of the Year 2023', 'Serie A top scorer 2022–23', 'Coppa Italia'],
    career: ['Wolfsburg', 'Lille', 'Napoli', 'Galatasaray'],
    careerStations: [{ team: 'Wolfsburg', period: '2017–2019', goals: 0, assists: 0, apps: 16 }, { team: 'Lille', period: '2019–2020', goals: 18, assists: 6, apps: 38 }, { team: 'Napoli', period: '2020–2024', goals: 76, assists: 12, apps: 133 }, { team: 'Galatasaray', period: '2024–', goals: 8, assists: 2, apps: 18 }, { team: 'Nigeria', period: '2017–', goals: 21, assists: 5, apps: 35 }],
    statsByCompetition: { 'Serie A': { goals: 67, assists: 10 }, 'Champions League': { goals: 10, assists: 2 }, 'Ligue 1': { goals: 13, assists: 5 }, 'International': { goals: 21, assists: 5 } },
    achievements: ['African Footballer of the Year', 'Serie A capocannoniere', 'Napoli legend'] },
  { id: 444222, name: 'Florian Wirtz', club: 'Leverkusen', position: 'midfielders', rating: 89, goals: 18, image: getHeadshot(444222), age: 21, height: '1.76m', weight: '70kg',
    trophies: ['1× Bundesliga 2023–24', '1× DFB-Pokal 2023–24', 'UEFA Europa League runner-up 2024', 'Bundesliga Player of the Season 2023–24'],
    career: ['Koln', 'Leverkusen'],
    careerStations: [{ team: 'Koln', period: '2020', goals: 1, assists: 0, apps: 2 }, { team: 'Leverkusen', period: '2020–', goals: 45, assists: 52, apps: 145 }, { team: 'Germany', period: '2021–', goals: 4, assists: 8, apps: 28 }],
    statsByCompetition: { 'Bundesliga': { goals: 35, assists: 42 }, 'Europa League': { goals: 5, assists: 6 }, 'International': { goals: 4, assists: 8 } },
    achievements: ['Bundesliga Player of the Season 2024', 'Invincible Bundesliga title', 'Euro 2024'] },
  { id: 410313, name: 'Jamal Musiala', club: 'Bayern Munich', position: 'midfielders', rating: 88, goals: 15, image: getHeadshot(410313), age: 21, height: '1.84m', weight: '72kg',
    trophies: ['4× Bundesliga', '1× UEFA Champions League', '3× DFL-Supercup', 'FIFA Club World Cup', 'Euro 2024'],
    career: ['Chelsea Academy', 'Bayern Munich'],
    careerStations: [{ team: 'Bayern Munich', period: '2020–', goals: 48, assists: 35, apps: 163 }, { team: 'Germany', period: '2021–', goals: 6, assists: 8, apps: 32 }],
    statsByCompetition: { 'Bundesliga': { goals: 38, assists: 28 }, 'Champions League': { goals: 8, assists: 6 }, 'International': { goals: 6, assists: 8 } },
    achievements: ['Germany youngest Euro player', 'Bundesliga Young Player', 'Euro 2024 winner'] },
  { id: 139871, name: 'Virgil van Dijk', club: 'Liverpool', position: 'defenders', rating: 89, goals: 4, image: getHeadshot(139871), age: 33, height: '1.93m', weight: '92kg',
    trophies: ['1× UEFA Champions League', '1× Premier League', '1× FA Cup', '1× League Cup', 'UEFA Men\'s Player of the Year 2019', 'PFA Player of the Year', 'Netherlands captain'],
    career: ['Groningen', 'Celtic', 'Southampton', 'Liverpool'],
    careerStations: [{ team: 'Groningen', period: '2011–2013', goals: 7, assists: 2, apps: 62 }, { team: 'Celtic', period: '2013–2015', goals: 9, assists: 5, apps: 76 }, { team: 'Southampton', period: '2015–2018', goals: 4, assists: 2, apps: 80 }, { team: 'Liverpool', period: '2018–', goals: 24, assists: 8, apps: 268 }, { team: 'Netherlands', period: '2015–', goals: 7, assists: 2, apps: 68 }],
    statsByCompetition: { 'Premier League': { goals: 18, assists: 6 }, 'Champions League': { goals: 4, assists: 2 }, 'Eredivisie': { goals: 5, assists: 2 }, 'International': { goals: 7, assists: 2 } },
    achievements: ['UEFA Men\'s Player of the Year', 'UCL winner 2019', 'PL winner 2020'] },
  { id: 253919, name: 'Declan Rice', club: 'Arsenal', position: 'midfielders', rating: 87, goals: 7, image: getHeadshot(253919), age: 25, height: '1.88m', weight: '80kg',
    trophies: ['1× UEFA Europa Conference League', 'Hammer of the Year (3×)', 'Arsenal key signing'],
    career: ['West Ham', 'Arsenal'],
    careerStations: [{ team: 'West Ham', period: '2017–2023', goals: 15, assists: 13, apps: 245 }, { team: 'Arsenal', period: '2023–', goals: 8, assists: 6, apps: 52 }, { team: 'England', period: '2019–', goals: 3, assists: 2, apps: 55 }],
    statsByCompetition: { 'Premier League': { goals: 18, assists: 14 }, 'Europa League': { goals: 2, assists: 2 }, 'Conference League': { goals: 1, assists: 1 }, 'International': { goals: 3, assists: 2 } },
    achievements: ['Hammer of the Year 3×', 'Euro 2020 runner-up', 'England regular'] },
  { id: 424911, name: 'Cole Palmer', club: 'Chelsea', position: 'midfielders', rating: 87, goals: 24, image: getHeadshot(424911), age: 22, height: '1.89m', weight: '74kg',
    trophies: ['1× UEFA Champions League', '1× Premier League', '1× Super Cup', 'PL Young Player of the Season 2023–24', 'Chelsea Player of the Season'],
    career: ['Man City', 'Chelsea'],
    careerStations: [{ team: 'Man City', period: '2021–2023', goals: 0, assists: 2, apps: 41 }, { team: 'Chelsea', period: '2023–', goals: 28, assists: 18, apps: 45 }, { team: 'England', period: '2023–', goals: 2, assists: 2, apps: 12 }],
    statsByCompetition: { 'Premier League': { goals: 25, assists: 16 }, 'Champions League': { goals: 2, assists: 2 }, 'International': { goals: 2, assists: 2 } },
    achievements: ['PL Young Player of the Season', 'Chelsea star 2023–24', 'Euro 2024'] },
  { id: 210411, name: 'Leroy Sané', club: 'Bayern', position: 'forwards', rating: 86, goals: 12, image: getHeadshot(210411), age: 28, height: '1.83m', weight: '80kg',
    trophies: ['2× Premier League', '3× Bundesliga', '1× FA Cup', '1× DFL-Supercup', 'PFA Young Player of the Year', 'Euro 2024'],
    career: ['Schalke', 'Man City', 'Bayern'],
    careerStations: [{ team: 'Schalke', period: '2014–2016', goals: 13, assists: 8, apps: 57 }, { team: 'Man City', period: '2016–2020', goals: 39, assists: 45, apps: 135 }, { team: 'Bayern Munich', period: '2020–', goals: 58, assists: 48, apps: 168 }, { team: 'Germany', period: '2015–', goals: 13, assists: 10, apps: 59 }],
    statsByCompetition: { 'Premier League': { goals: 25, assists: 28 }, 'Bundesliga': { goals: 48, assists: 42 }, 'Champions League': { goals: 12, assists: 10 }, 'International': { goals: 13, assists: 10 } },
    achievements: ['PFA Young Player of the Year', 'Euro 2024', 'Bayern winger'] },
  { id: 191695, name: 'Bernardo Silva', club: 'Man City', position: 'midfielders', rating: 88, goals: 11, image: getHeadshot(191695), age: 29,
    trophies: ['1× UEFA Champions League', '6× Premier League', '2× FA Cup', '4× League Cup', 'FIFA Club World Cup', 'UEFA Nations League 2019'],
    career: ['Benfica', 'Monaco', 'Man City'],
    careerStations: [{ team: 'Benfica', period: '2013–2017', goals: 21, assists: 28, apps: 112 }, { team: 'Monaco', period: '2014–2017', goals: 22, assists: 22, apps: 101 }, { team: 'Man City', period: '2017–', goals: 68, assists: 62, apps: 358 }, { team: 'Portugal', period: '2015–', goals: 11, assists: 24, apps: 92 }],
    statsByCompetition: { 'Premier League': { goals: 48, assists: 48 }, 'Champions League': { goals: 12, assists: 12 }, 'Ligue 1': { goals: 18, assists: 18 }, 'International': { goals: 11, assists: 24 } },
    achievements: ['Elite playmaker', 'Treble winner', 'Portugal star'] },
  { id: 238864, name: 'Phil Foden', club: 'Man City', position: 'midfielders', rating: 90, goals: 28, image: getHeadshot(238864), age: 24,
    trophies: ['1× UEFA Champions League', '6× Premier League', '2× FA Cup', '4× League Cup', 'FIFA Club World Cup', 'PL Young Player of the Season (2×)', 'PL Player of the Season 2023–24'],
    career: ['Man City'],
    careerStations: [{ team: 'Man City', period: '2017–', goals: 92, assists: 52, apps: 278 }, { team: 'England', period: '2020–', goals: 4, assists: 4, apps: 38 }],
    statsByCompetition: { 'Premier League': { goals: 72, assists: 42 }, 'Champions League': { goals: 14, assists: 8 }, 'FA Cup': { goals: 6, assists: 2 }, 'International': { goals: 4, assists: 4 } },
    achievements: ['PL Player of the Season 2024', 'Treble winner', 'Euro 2024'] },
  { id: 104230, name: 'Manuel Neuer', club: 'Bayern Munich', position: 'goalkeepers', rating: 87, goals: 0, image: getHeadshot(104230), age: 38,
    trophies: ['2× UEFA Champions League', '12× Bundesliga', '1× FIFA World Cup 2014', '6× DFB-Pokal', '2× FIFA Club World Cup', 'UEFA Goalkeeper of the Year'],
    career: ['Schalke', 'Bayern'],
    careerStations: [{ team: 'Schalke', period: '2006–2011', goals: 0, assists: 0, apps: 203 }, { team: 'Bayern Munich', period: '2011–', goals: 0, assists: 0, apps: 511 }, { team: 'Germany', period: '2009–', goals: 0, assists: 0, apps: 117 }],
    statsByCompetition: { 'Bundesliga': { goals: 0, assists: 0 }, 'Champions League': { goals: 0, assists: 0 }, 'International': { goals: 0, assists: 0 } },
    achievements: ['Sweeper-keeper pioneer', 'World Cup 2014', 'Bayern legend'] },
  { id: 394392, name: 'Rafael Leão', club: 'AC Milan', position: 'forwards', rating: 86, goals: 15, image: getHeadshot(394392), age: 25, height: '1.88m', weight: '81kg', trophies: ['1× Serie A 2021–22', 'Serie A Footballer of the Year'], career: ['Sporting CP', 'Lille', 'AC Milan'], achievements: ['Serie A Footballer of the Year'],
    careerStations: [{ team: 'Sporting CP', period: '2018', goals: 1, assists: 0, apps: 5 }, { team: 'Lille', period: '2019–2021', goals: 8, assists: 8, apps: 84 }, { team: 'AC Milan', period: '2021–', goals: 58, assists: 38, apps: 165 }, { team: 'Portugal', period: '2021–', goals: 4, assists: 4, apps: 28 }],
    statsByCompetition: { 'Serie A': { goals: 48, assists: 32 }, 'Ligue 1': { goals: 6, assists: 6 }, 'Champions League': { goals: 6, assists: 4 }, 'International': { goals: 4, assists: 4 } } },
  { id: 391157, name: 'Dusan Vlahovic', club: 'Juventus', position: 'forwards', rating: 84, goals: 18, image: getHeadshot(391157), age: 24, height: '1.90m', weight: '75kg', trophies: ['1× Coppa Italia', 'Serie A Best Young Player'], career: ['Partizan', 'Fiorentina', 'Juventus'], achievements: ['Serie A Best Young Player'],
    careerStations: [{ team: 'Partizan', period: '2016–2018', goals: 1, assists: 0, apps: 21 }, { team: 'Fiorentina', period: '2018–2022', goals: 49, assists: 8, apps: 108 }, { team: 'Juventus', period: '2022–', goals: 42, assists: 8, apps: 98 }, { team: 'Serbia', period: '2020–', goals: 14, assists: 2, apps: 28 }],
    statsByCompetition: { 'Serie A': { goals: 78, assists: 14 }, 'Champions League': { goals: 10, assists: 2 }, 'International': { goals: 14, assists: 2 } } },
  { id: 233777, name: 'Nicolo Barella', club: 'Inter', position: 'midfielders', rating: 87, goals: 3, image: getHeadshot(233777), age: 27, height: '1.72m', weight: '68kg', trophies: ['1× UEFA Euro 2020', '2× Serie A', '2× Coppa Italia', 'Serie A Best Midfielder (3×)'], career: ['Cagliari', 'Inter Milan'], achievements: ['Serie A Best Midfielder 3×'],
    careerStations: [{ team: 'Cagliari', period: '2015–2020', goals: 7, assists: 6, apps: 112 }, { team: 'Inter Milan', period: '2020–', goals: 22, assists: 42, apps: 195 }, { team: 'Italy', period: '2018–', goals: 9, assists: 8, apps: 60 }],
    statsByCompetition: { 'Serie A': { goals: 24, assists: 42 }, 'Champions League': { goals: 4, assists: 6 }, 'International': { goals: 9, assists: 8 } } },
  { id: 139870, name: 'Thibaut Courtois', club: 'Real Madrid', position: 'goalkeepers', rating: 90, goals: 0, image: getHeadshot(139870), age: 32, height: '2.00m', weight: '96kg', trophies: ['2× UEFA Champions League', '3× La Liga', '2× Premier League', 'Yashin Trophy 2022', 'La Liga 2023–24'], career: ['Genk', 'Atletico', 'Chelsea', 'Real Madrid'], achievements: ['Yashin Trophy 2022'],
    careerStations: [{ team: 'Genk', period: '2009–2011', goals: 0, assists: 0, apps: 45 }, { team: 'Atletico Madrid', period: '2011–2014', goals: 0, assists: 0, apps: 154 }, { team: 'Chelsea', period: '2014–2018', goals: 0, assists: 0, apps: 154 }, { team: 'Real Madrid', period: '2018–', goals: 0, assists: 0, apps: 230 }, { team: 'Belgium', period: '2011–', goals: 0, assists: 0, apps: 102 }],
    statsByCompetition: { 'La Liga': { goals: 0, assists: 0 }, 'Premier League': { goals: 0, assists: 0 }, 'Champions League': { goals: 0, assists: 0 }, 'International': { goals: 0, assists: 0 } } },
  { id: 175654, name: 'Alisson', club: 'Liverpool', position: 'goalkeepers', rating: 89, goals: 0, image: getHeadshot(175654), age: 32, height: '1.93m', weight: '91kg', trophies: ['1× UEFA Champions League', '1× Premier League', '1× Copa América 2019', '2× League Cup', 'FA Cup', 'The Best FIFA Goalkeeper'], career: ['Internacional', 'Roma', 'Liverpool'], achievements: ['The Best FIFA Goalkeeper'],
    careerStations: [{ team: 'Internacional', period: '2013–2016', goals: 0, assists: 0, apps: 80 }, { team: 'Roma', period: '2016–2018', goals: 0, assists: 0, apps: 64 }, { team: 'Liverpool', period: '2018–', goals: 1, assists: 0, apps: 268 }, { team: 'Brazil', period: '2015–', goals: 0, assists: 0, apps: 65 }],
    statsByCompetition: { 'Premier League': { goals: 1, assists: 0 }, 'Champions League': { goals: 0, assists: 0 }, 'Serie A': { goals: 0, assists: 0 }, 'International': { goals: 0, assists: 0 } } },
  { id: 233775, name: 'Rúben Dias', club: 'Man City', position: 'defenders', rating: 89, goals: 4, image: getHeadshot(233775), age: 27, trophies: ['1× UEFA Champions League', '4× Premier League', '2× FA Cup', 'FIFA Club World Cup', 'PL Player of the Season 2020–21', 'UEFA Nations League 2019'], career: ['Benfica', 'Man City'], achievements: ['PL Player of the Season'],
    careerStations: [{ team: 'Benfica', period: '2017–2020', goals: 3, assists: 2, apps: 91 }, { team: 'Man City', period: '2020–', goals: 8, assists: 4, apps: 185 }, { team: 'Portugal', period: '2018–', goals: 2, assists: 0, apps: 58 }],
    statsByCompetition: { 'Premier League': { goals: 6, assists: 4 }, 'Champions League': { goals: 2, assists: 0 }, 'Primeira Liga': { goals: 2, assists: 2 }, 'International': { goals: 2, assists: 0 } } },
  { id: 211019, name: 'Ederson', club: 'Man City', position: 'goalkeepers', rating: 88, goals: 0, image: getHeadshot(211019), age: 30, trophies: ['1× UEFA Champions League', '6× Premier League', '2× FA Cup', '4× League Cup', 'FIFA Club World Cup', 'Premier League Golden Glove (4×)'], career: ['Benfica', 'Man City'], achievements: ['Golden Glove winner'],
    careerStations: [{ team: 'Benfica', period: '2015–2017', goals: 0, assists: 0, apps: 37 }, { team: 'Man City', period: '2017–', goals: 0, assists: 0, apps: 332 }, { team: 'Brazil', period: '2017–', goals: 0, assists: 0, apps: 28 }],
    statsByCompetition: { 'Premier League': { goals: 0, assists: 0 }, 'Champions League': { goals: 0, assists: 0 }, 'International': { goals: 0, assists: 0 } } },
  { id: 186001, name: 'Joshua Kimmich', club: 'Bayern Munich', position: 'midfielders', rating: 88, goals: 42, image: getHeadshot(186001), age: 29, trophies: ['1× UEFA Champions League', '8× Bundesliga', '6× DFB-Pokal', 'FIFA Club World Cup', 'Euro 2024'], career: ['Leipzig', 'Bayern'], achievements: ['Modern versatility master'],
    careerStations: [{ team: 'Leipzig', period: '2013–2015', goals: 0, assists: 0, apps: 53 }, { team: 'Bayern Munich', period: '2015–', goals: 42, assists: 102, apps: 398 }, { team: 'Germany', period: '2016–', goals: 6, assists: 14, apps: 88 }],
    statsByCompetition: { 'Bundesliga': { goals: 35, assists: 88 }, 'Champions League': { goals: 5, assists: 12 }, 'International': { goals: 6, assists: 14 } } },
...
const EXTRA_SPORT_PLAYERS = {
  ...
  cricket: [
    // === INDIA ===
    { id: 253802, name: 'Virat Kohli', club: 'India / RCB', position: 'Batter', rating: 97, goals: 26733, assists: 80, image: getCricketHeadshot(253802), age: 36, height: '1.75m', weight: '69kg', career: ['India', 'Royal Challengers Bangalore'], trophies: ['ICC Champions Trophy 2013', 'ICC Test Team of Decade', 'ICC ODI Team of Decade', 'Multiple IPL Orange Cap', 'T20 World Cup 2024'], achievements: ['50+ ODI centuries', 'One of the all-time ODI greats', 'India run-machine'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s',
      careerStations: [{ team: 'India (Test)', period: '2011–', goals: 8848, assists: 29, apps: 113 }, { team: 'India (ODI)', period: '2008–', goals: 13848, assists: 50, apps: 292 }, { team: 'India (T20I)', period: '2010–', goals: 4037, assists: 1, apps: 117 }, { team: 'RCB (IPL)', period: '2008–', goals: 8004, assists: 0, apps: 252 }],
      statsByCompetition: { 'Test': { goals: 8848, assists: 29 }, 'ODI': { goals: 13848, assists: 50 }, 'T20I': { goals: 4037, assists: 1 }, 'IPL': { goals: 8004, assists: 0 } } },
    { id: 34102, name: 'Rohit Sharma', club: 'India / MI', position: 'Batter', rating: 95, goals: 18437, assists: 49, image: getCricketHeadshot(34102), age: 37, height: '1.74m', weight: '72kg', career: ['India', 'Mumbai Indians'], trophies: ['5× IPL Champion', 'Asia Cup', 'ICC ODI Team of Decade', 'T20 World Cup 2024'], achievements: ['Highest ODI score (264)', 'Double-century record holder', 'India white-ball captain'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s',
      careerStations: [{ team: 'India (Test)', period: '2013–', goals: 4137, assists: 10, apps: 59 }, { team: 'India (ODI)', period: '2007–', goals: 10709, assists: 31, apps: 262 }, { team: 'India (T20I)', period: '2007–', goals: 4234, assists: 5, apps: 151 }, { team: 'Mumbai Indians (IPL)', period: '2011–', goals: 5357, assists: 2, apps: 213 }],
      statsByCompetition: { 'Test': { goals: 4137, assists: 10 }, 'ODI': { goals: 10709, assists: 31 }, 'T20I': { goals: 4234, assists: 5 }, 'IPL': { goals: 5357, assists: 2 } } },
    { id: 625383, name: 'Jasprit Bumrah', club: 'India / MI', position: 'Bowler', rating: 96, goals: 427, assists: 9, image: getCricketHeadshot(625383), age: 31, height: '1.78m', weight: '68kg', career: ['India', 'Mumbai Indians'], trophies: ['ICC #1 Test Bowler', 'IPL Champion'], achievements: ['Elite yorker specialist', 'All-format spearhead'], primaryStatLabel: 'Wickets', secondaryStatLabel: '5WI',
      careerStations: [{ team: 'India (Test)', period: '2018–', goals: 159, assists: 10, apps: 36 }, { team: 'India (ODI)', period: '2016–', goals: 149, assists: 2, apps: 89 }, { team: 'India (T20I)', period: '2016–', goals: 74, assists: 0, apps: 62 }, { team: 'Mumbai Indians (IPL)', period: '2013–', goals: 145, assists: 0, apps: 130 }],
      statsByCompetition: { 'Test': { goals: 159, assists: 10 }, 'ODI': { goals: 149, assists: 2 }, 'T20I': { goals: 74, assists: 0 }, 'IPL': { goals: 145, assists: 0 } } },
    { id: 10617, name: 'MS Dhoni', club: 'India / CSK', position: 'Wicketkeeper', rating: 96, goals: 17266, assists: 0, image: getCricketHeadshot(10617), age: 43, height: '1.75m', weight: '72kg', career: ['India', 'Chennai Super Kings'], trophies: ['2× World Cup', '5× IPL', 'Champions Trophy', 'ICC World T20 2007', 'T20 World Cup 2024 (India)'], achievements: ['Legendary finisher & captain', 'Most IPL titles as captain'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Dismissals',
      careerStations: [{ team: 'India (Test)', period: '2005–2014', goals: 4876, assists: 0, apps: 90 }, { team: 'India (ODI)', period: '2004–2019', goals: 10773, assists: 0, apps: 350 }, { team: 'India (T20I)', period: '2006–2019', goals: 1617, assists: 0, apps: 98 }, { team: 'CSK (IPL)', period: '2008–', goals: 5162, assists: 0, apps: 250 }],
      statsByCompetition: { 'Test': { goals: 4876, assists: 0 }, 'ODI': { goals: 10773, assists: 0 }, 'T20I': { goals: 1617, assists: 0 }, 'IPL': { goals: 5162, assists: 0 } } },
    { id: 422108, name: 'KL Rahul', club: 'India / LSG', position: 'Batter', rating: 90, goals: 9500, assists: 0, image: getCricketHeadshot(422108), age: 32, height: '1.80m', weight: '72kg', career: ['India', 'Lucknow Super Giants'], trophies: ['Asia Cup'], achievements: ['Elite opener across formats', 'IPL Orange Cap'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s',
      careerStations: [{ team: 'India (Test)', period: '2014–', goals: 2842, assists: 0, apps: 50 }, { team: 'India (ODI)', period: '2016–', goals: 2810, assists: 0, apps: 72 }, { team: 'India (T20I)', period: '2016–', goals: 2265, assists: 0, apps: 72 }, { team: 'LSG (IPL)', period: '2022–', goals: 1583, assists: 0, apps: 45 }],
      statsByCompetition: { 'Test': { goals: 2842, assists: 0 }, 'ODI': { goals: 2810, assists: 0 }, 'T20I': { goals: 2265, assists: 0 }, 'IPL': { goals: 1583, assists: 0 } } },
    ...
    { id: 425943, name: 'Sanju Samson', club: 'India / RR', position: 'Wicketkeeper', rating: 89, goals: 5500, assists: 0, image: getCricketHeadshot(425943), age: 30, height: '1.78m', weight: '72kg', career: ['India', 'Rajasthan Royals'], trophies: ['T20 World Cup 2024', 'IPL runner-up'], achievements: ['India wicketkeeper-batter', 'Elegant strokeplayer'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Dismissals' },
    { id: 720471, name: 'Ishan Kishan', club: 'India / MI', position: 'Wicketkeeper', rating: 88, goals: 3500, assists: 0, image: getCricketHeadshot(720471), age: 26, height: '1.68m', weight: '62kg', career: ['India', 'Mumbai Indians'], trophies: ['Asia Cup 2023', 'U19 World Cup captain'], achievements: ['Fastest ODI double-century (210)', 'Aggressive keeper-batter'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Dismissals' },
    { id: 446507, name: 'Suryakumar Yadav', club: 'India / MI', position: 'Batter', rating: 92, goals: 5500, assists: 0, image: getCricketHeadshot(446507), age: 34, height: '1.78m', weight: '72kg', career: ['India', 'Mumbai Indians'], trophies: ['T20 World Cup 2024', '5x IPL'], achievements: ['ICC T20I #1 batter', '360° strokeplay'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s' },
    { id: 1151278, name: 'Yashasvi Jaiswal', club: 'India / RR', position: 'Batter', rating: 89, goals: 2500, assists: 0, image: getCricketHeadshot(1151278), age: 22, height: '1.78m', weight: '68kg', career: ['India', 'Rajasthan Royals'], trophies: ['T20 World Cup 2024', 'IPL Orange Cap 2024'], achievements: ['Double century in Tests', 'Next-gen opener'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s' },
    { id: 1060380, name: 'Ruturaj Gaikwad', club: 'India / CSK', position: 'Batter', rating: 88, goals: 3500, assists: 0, image: getCricketHeadshot(1060380), age: 27, height: '1.78m', weight: '72kg', career: ['India', 'Chennai Super Kings'], trophies: ['IPL 2024', 'Orange Cap 2021'], achievements: ['CSK captain', 'Elegant opener'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s' },
    { id: 554691, name: 'Axar Patel', club: 'India / DC', position: 'All-rounder', rating: 87, goals: 2500, assists: 185, image: getCricketHeadshot(554691), age: 30, height: '1.83m', weight: '78kg', career: ['India', 'Delhi Capitals'], trophies: ['Asia Cup', 'Border-Gavaskar hero'], achievements: ['Left-arm spin & lower-order runs', 'Test match-winner'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Wkts' },
    { id: 430246, name: 'Yuzvendra Chahal', club: 'India / RR', position: 'Bowler', rating: 88, goals: 335, assists: 2, image: getCricketHeadshot(430246), age: 34, height: '1.68m', weight: '62kg', career: ['India', 'Rajasthan Royals'], trophies: ['IPL Purple Cap', '2x IPL'], achievements: ['Leading IPL wicket-taker', 'Leg-spin wizard'], primaryStatLabel: 'Wickets', secondaryStatLabel: '4WI' },
    { id: 1125976, name: 'Arshdeep Singh', club: 'India / PBKS', position: 'Bowler', rating: 86, goals: 85, assists: 2, image: getCricketHeadshot(1125976), age: 25, height: '1.88m', weight: '78kg', career: ['India', 'Punjab Kings'], trophies: ['T20 World Cup 2024', 'Asia Cup'], achievements: ['Left-arm swing', 'Death-overs specialist'], primaryStatLabel: 'Wickets', secondaryStatLabel: '4WI' },
    { id: 1070168, name: 'Prithvi Shaw', club: 'India / DC', position: 'Batter', rating: 85, goals: 2500, assists: 0, image: getCricketHeadshot(1070168), age: 24, height: '1.68m', weight: '62kg', career: ['India', 'Delhi Capitals'], trophies: ['U19 World Cup 2018'], achievements: ['Century on Test debut', 'Explosive opener'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s' },
    { id: 719715, name: 'Washington Sundar', club: 'India / SRH', position: 'All-rounder', rating: 84, goals: 1200, assists: 95, image: getCricketHeadshot(719715), age: 24, height: '1.83m', weight: '72kg', career: ['India', 'Sunrisers Hyderabad'], trophies: ['Border-Gavaskar 2021'], achievements: ['Off-spin & lower-order bat', 'Test debut hero'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Wkts' },
    { id: 1243990, name: 'Umran Malik', club: 'India / KKR', position: 'Bowler', rating: 85, goals: 45, assists: 2, image: getCricketHeadshot(1243990), age: 24, height: '1.78m', weight: '72kg', career: ['India', 'Kolkata Knight Riders'], trophies: ['IPL 2022 emerging star'], achievements: ['155kph+ pace', 'Jammu Express'], primaryStatLabel: 'Wickets', secondaryStatLabel: '4WI' },
    { id: 475281, name: 'Shardul Thakur', club: 'India / KKR', position: 'All-rounder', rating: 84, goals: 800, assists: 135, image: getCricketHeadshot(475281), age: 33, height: '1.78m', weight: '78kg', career: ['India', 'Kolkata Knight Riders'], trophies: ['Asia Cup', 'WTC'], achievements: ['Lord Shardul', 'Crucial lower-order runs & wickets'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Wkts' },
    // === AUSTRALIA ===
    { id: 28081, name: 'Pat Cummins', club: 'Australia / SRH', position: 'Bowler', rating: 95, goals: 398, assists: 8, image: getCricketHeadshot(28081), age: 31, height: '1.92m', weight: '82kg', career: ['Australia', 'Sunrisers Hyderabad'], trophies: ['World Cup 2023', 'WTC', 'T20 World Cup 2021', 'ICC Men\'s Cricketer of the Year', 'Australian captain'], achievements: ['Australian captain', 'All-format leader'], primaryStatLabel: 'Wickets', secondaryStatLabel: '5WI',
      careerStations: [{ team: 'Australia (Test)', period: '2011–', goals: 269, assists: 11, apps: 63 }, { team: 'Australia (ODI)', period: '2011–', goals: 141, assists: 1, apps: 88 }, { team: 'Australia (T20I)', period: '2011–', goals: 58, assists: 0, apps: 54 }, { team: 'SRH (IPL)', period: '2024–', goals: 17, assists: 0, apps: 16 }],
      statsByCompetition: { 'Test': { goals: 269, assists: 11 }, 'ODI': { goals: 141, assists: 1 }, 'T20I': { goals: 58, assists: 0 }, 'IPL': { goals: 17, assists: 0 } } },
    ...
    // === ENGLAND ===
    { id: 232364, name: 'Joe Root', club: 'England / TKR', position: 'Batter', rating: 94, goals: 19000, assists: 62, image: getCricketHeadshot(232364), age: 34, height: '1.83m', weight: '80kg', career: ['England', 'Yorkshire'], trophies: ['T20 World Cup 2022', 'Ashes', 'England Test captain', 'ICC Cricketer of the Year'], achievements: ['England all-time Test run-scorer', 'All-format class'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s',
      careerStations: [{ team: 'England (Test)', period: '2012–', goals: 11420, assists: 30, apps: 140 }, { team: 'England (ODI)', period: '2013–', goals: 6522, assists: 16, apps: 171 }, { team: 'England (T20I)', period: '2012–', goals: 893, assists: 0, apps: 32 }, { team: 'Yorkshire', period: '2009–', goals: 15000, assists: 0, apps: 200 }],
      statsByCompetition: { 'Test': { goals: 11420, assists: 30 }, 'ODI': { goals: 6522, assists: 16 }, 'T20I': { goals: 893, assists: 0 } } },
    { id: 230853, name: 'Ben Stokes', club: 'England / CSK', position: 'All-rounder', rating: 94, goals: 12500, assists: 235, image: getCricketHeadshot(230853), age: 33, height: '1.85m', weight: '85kg', career: ['England', 'Chennai Super Kings'], trophies: ['World Cup 2019', 'T20 World Cup 2022'], achievements: ['Headingley 2019', 'Captain in all formats'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Wkts',
      careerStations: [{ team: 'England (Test)', period: '2013–', goals: 6318, assists: 197, apps: 102 }, { team: 'England (ODI)', period: '2011–', goals: 3464, assists: 74, apps: 114 }, { team: 'England (T20I)', period: '2011–', goals: 734, assists: 28, apps: 43 }, { team: 'CSK (IPL)', period: '2023–', goals: 450, assists: 15, apps: 25 }],
      statsByCompetition: { 'Test': { goals: 6318, assists: 197 }, 'ODI': { goals: 3464, assists: 74 }, 'T20I': { goals: 734, assists: 28 }, 'IPL': { goals: 450, assists: 15 } } },
    ...
    { id: 1194959, name: 'Jacob Bethell', club: 'England / RCB', position: 'All-rounder', rating: 84, goals: 800, assists: 25, image: getCricketHeadshot(1194959), age: 21, height: '1.80m', weight: '75kg', career: ['England', 'Warwickshire', 'RCB'], trophies: ['England T20I captain'], achievements: ['Youngest England T20I captain', 'Explosive left-handed batter'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Wkts' },
    // === PAKISTAN ===
    { id: 348144, name: 'Babar Azam', club: 'Pakistan / PZ', position: 'Batter', rating: 93, goals: 13000, assists: 0, image: getCricketHeadshot(348144), age: 30, height: '1.80m', weight: '70kg', career: ['Pakistan', 'Peshawar Zalmi'], trophies: ['ICC #1 ODI batter', 'PSL'], achievements: ['Elegant all-format run-machine'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s',
      careerStations: [{ team: 'Pakistan (Test)', period: '2016–', goals: 3892, assists: 9, apps: 52 }, { team: 'Pakistan (ODI)', period: '2015–', goals: 5729, assists: 19, apps: 117 }, { team: 'Pakistan (T20I)', period: '2016–', goals: 4023, assists: 3, apps: 123 }, { team: 'Peshawar Zalmi (PSL)', period: '2016–', goals: 3000, assists: 0, apps: 85 }],
      statsByCompetition: { 'Test': { goals: 3892, assists: 9 }, 'ODI': { goals: 5729, assists: 19 }, 'T20I': { goals: 4023, assists: 3 }, 'PSL': { goals: 3000, assists: 0 } } },
    ...
    { id: 277916, name: 'Kane Williamson', club: 'New Zealand / GT', position: 'Batter', rating: 94, goals: 17500, assists: 45, image: getCricketHeadshot(277916), age: 34, height: '1.80m', weight: '82kg', career: ['New Zealand', 'Gujarat Titans'], trophies: ['WTC Champion', 'ICC Spirit of Cricket'], achievements: ['NZ leading run-scorer', 'Elite in all formats'], primaryStatLabel: 'Runs', secondaryStatLabel: '100s',
      careerStations: [{ team: 'New Zealand (Test)', period: '2010–', goals: 8672, assists: 32, apps: 100 }, { team: 'New Zealand (ODI)', period: '2010–', goals: 6910, assists: 13, apps: 169 }, { team: 'New Zealand (T20I)', period: '2011–', goals: 2465, assists: 0, apps: 87 }, { team: 'Gujarat Titans (IPL)', period: '2022–', goals: 1453, assists: 0, apps: 42 }],
      statsByCompetition: { 'Test': { goals: 8672, assists: 32 }, 'ODI': { goals: 6910, assists: 13 }, 'T20I': { goals: 2465, assists: 0 }, 'IPL': { goals: 1453, assists: 0 } } },
    { id: 959767, name: 'Rachin Ravindra', club: 'New Zealand / CSK', position: 'All-rounder', rating: 90, goals: 2500, assists: 45, image: getCricketHeadshot(959767), age: 25, height: '1.78m', weight: '72kg', career: ['New Zealand', 'Chennai Super Kings'], trophies: ['T20 World Cup 2024', 'World Cup 2023 star'], achievements: ['World Cup 2023 breakout', 'Left-hand bat & spin'], primaryStatLabel: 'Runs', secondaryStatLabel: 'Wkts' },
    ...
    { id: 493773, name: 'Lockie Ferguson', club: 'New Zealand / RCB', position: 'Bowler', rating: 88, goals: 185, assists: 4, image: getCricketHeadshot(493773), age: 33, height: '1.80m', weight: '82kg', career: ['New Zealand', 'Royal Challengers Bangalore'], trophies: ['T20 World Cup 2021', 'IPL'], achievements: ['Express pace', '150kph+'], primaryStatLabel: 'Wickets', secondaryStatLabel: '4WI' },
    // === SOUTH AFRICA ===
    ...
    { id: 379145, name: 'Tabraiz Shamsi', club: 'South Africa', position: 'Bowler', rating: 86, goals: 185, assists: 2, image: getCricketHeadshot(379145), age: 34, height: '1.78m', weight: '72kg', career: ['South Africa'], trophies: ['SA20'], achievements: ['Left-arm wrist-spin', 'T20I wicket record'], primaryStatLabel: 'Wickets', secondaryStatLabel: '4WI' },
    { id: 44933, name: 'Lungi Ngidi', club: 'South Africa / DC', position: 'Bowler', rating: 85, goals: 95, assists: 2, image: getCricketHeadshot(44933), age: 28, height: '1.91m', weight: '92kg', career: ['South Africa', 'Delhi Capitals'], trophies: ['SA20'], achievements: ['Pace & bounce', 'T20 World Cup star'], primaryStatLabel: 'Wickets', secondaryStatLabel: '4WI' },
    // === WEST INDIES ===
...
const HomePage = ({ isAuthenticated = false, homeTheme = 'light', setHomeTheme, enabledSportKeys = [] }) => {
  const allSportKeys = ['soccer', 'cricket', 'f1', 'basketball', 'tennis', 'american-football', 'baseball', 'rugby', 'volleyball', 'badminton', 'mma', 'boxing', 'golf', 'swimming', 'cycling'];
  const homeToConfigKey = (key) => (key === 'american-football' ? 'football' : key);
  const enabledSet = new Set(Array.isArray(enabledSportKeys) ? enabledSportKeys : []);
  const sportKeys = enabledSet.size > 0 ? allSportKeys.filter((key) => enabledSet.has(homeToConfigKey(key))) : allSportKeys;
              ...
              <Link to="/signup" key={`${sport.key}-${i}`} className={`home-scard home-scard--ref home-scard--${sport.gradient}${sport.comingSoon ? ' home-scard--coming-soon' : ''}`}>
                <div className="home-scard-flip">
                  <div className="home-scard-face home-scard-front">
                    {sport.key === 'f1' && (
                      <video
                        className="home-scard-front-video"
                        src={`/videos/f1.mp4`}
                        muted
                        loop
                        autoPlay
                        ...
                        onTimeUpdate={(e) => {
                          if (e.target.currentTime >= 8) e.target.currentTime = 3;
                        }}
                      />
                    )}
                    {sport.avatar && sport.key !== 'f1' && (
                ...
                <div className="login-body">
                  ...
                  <div className="social-auth-grid">
                    <button type="button" className="google-auth-btn google-auth-btn--curly" onClick={() => handleGoogleLogin()}>
                      <span className="social-icon-box">
                        <img loading="lazy" decoding="async" src="https://www.google.com/favicon.ico" alt="" />
                      </span>
                  ...
                  <form onSubmit={handleEmailAuth} className="email-auth-form">
                    {isRegistering && (
                      <div className="login-field-wrap">
                        ...
                        <div className="input-group-pro input-group-pro--curly">
                          <User size={18} className="lucide-icon" aria-hidden="true" />
                          <input
                    ...
                    <div className="login-field-wrap">
                      ...
                      <div className="input-group-pro input-group-pro--curly">
                        <Mail size={18} className="lucide-icon" aria-hidden="true" />
                        <input
                          id="login-email"
                          type="email"
                    ...
                    <div className="login-field-wrap">
                      ...
                      <div className="input-group-pro input-group-pro--curly">
                        <Lock size={18} className="lucide-icon" aria-hidden="true" />
                        <input
                          id="login-password"
                          type="password"
    ...
    <aside className={`sidebar ${collapsed ? 'sidebar--collapsed' : ''}`} aria-label="Main navigation">
      ...
      <div className="sidebar-leagues-row">
        <nav className="nav-menu">
          ...
          {featureFlags.live_scores !== false && Object.keys(leagueNames).map(key => (
            <button key={key} className={`nav-item nav-item-league ${currentTab === key ? 'active' : ''}`} onClick={() => setTab(key)} title={leagueNames[key]}>
              <span className="nav-icon-wrap" aria-hidden="true">
                <img loading="lazy" decoding="async" src={leagueLogoUrl(key)} className="nav-icon league-logo-img" alt="" onError={(e) => { e.target.onerror = null; e.target.src = (selectedSport === 'cricket' ? CRICKET_LEAGUE_ICON : FALLBACK_LEAGUE_LOGO); }} />
              </span>
          ...
          {(selectedSport === 'basketball' || selectedSport === 'f1') && (
            <button className={`nav-item ${currentTab === 'minigames' ? 'active' : ''}`} onClick={() => setTab('minigames')} title="Mini games">
              <span className="material-icons-round">sports_esports</span>
              <span className="nav-item-text">Mini games</span>
            </button>
          )}
...
const MatchCard = ({ match, favorites, toggleFavorite, onOpen, showFavorite = true, hideLeagueLabel = false }) => {
  ...
  const isF1Race = match.isF1Race === true;

  if (isF1Race) {
    return (
      <div className="match-card match-card--f1 animate-in" onClick={() => onOpen(match)}>
        {match.isLive && <div className="live-badge"><div className="dot"></div>LIVE {match.status}</div>}
        {!hideLeagueLabel && <span className="league-label">{match.league}</span>}
        <div className="match-card-f1-single">
          <div className="team-info">
            <img loading="lazy" decoding="async" src={match.homeLogo} className="team-logo" alt="" onError={(e) => e.target.src = 'https://via.placeholder.com/32'} />
        ...
        <div className="match-time"><span className="material-icons-round" style={{ fontSize: '14px' }}>schedule</span> {match.time}</div>
        {match.isCompleted && match.status && (
          <div className="match-status-complete">{match.status}</div>
        )}
      </div>
    );
  }
...
function App() {
  ...
  // Real-time app config (feature flags, admins, maintenance, enabled sports) from Supabase.
  ...
  // Ensure Super Admin's user doc has role: 'super_admin' so RLS policies allow config writes
  useEffect(() => {
  ...
  // Supabase Auth + real-time user data
  useEffect(() => {
            ...
            setUserDataState((prev) => {
              ...
              if (serverHasSurveyData) {
                if (recentlyWroteSurvey && prev != null) {
                  // Trust local state for a short window after any write to prevent stale realtime snapshots from reverting changes.
  ...
  // Update lastSeen in Supabase so admins can see Active/Offline. Run on mount, on focus, and every 2 min.
  useEffect(() => {
    const uid = user?.uid;
    if (!uid) return;
  ...
  /** When the Top Players list was last refreshed from API (for "Updated Xs ago" live label). */
  const [lastPlayersRefreshAt, setLastPlayersRefreshAt] = useState(null);
  /** Seconds since last refresh; updates every second when on Players tab for live feel. */
  const [secondsSinceLastRefresh, setSecondsSinceLastRefresh] = useState(0);
  /** Live stats (goals, assists, runs, wickets, age) keyed by sport then player id; merged into displayed players. */
  ...
  const [nbaConferenceTab, setNbaConferenceTab] = useState('east'); // 'east' | 'west'
  const [worldcupTab, setWorldcupTab] = useState('groups'); // 'groups' | 'matches' | 'knockout'
  const [worldCupKnockoutMatches, setWorldCupKnockoutMatches] = useState([]);
  const [f1Tab, setF1Tab] = useState('overview'); // 'overview' | 'schedule' | 'live' | 'results' | 'drivers' | 'teams' | 'standings'
  /** Cache for Wikipedia fallback images by player name so we don't re-fetch */
  const wikipediaImageCacheRef = useRef({});
  /** Cricket: pre-fetched Wikipedia thumbnail URLs by player id (so ~23 without ESPN/Commons get an image) */
  const [cricketWikiUrls, setCricketWikiUrls] = useState({});

  /** When ESPN/primary image fails, fetch real photo from Wikipedia by player name (for all ~120 football players). */
  const setWikipediaFallbackImage = useCallback((playerName, imgEl) => {
    if (!imgEl || !playerName) { imgEl.src = FALLBACK_PLAYER_IMAGE; return; }
    const cached = wikipediaImageCacheRef.current[playerName];
    if (cached) { imgEl.src = cached; return; }
    const nameForUrl = encodeURIComponent(playerName.replace(/\s+/g, ' ').trim());
    fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${nameForUrl}`)
      .then((r) => r.ok ? r.json() : Promise.reject())
      .then((data) => {
        const src = data?.thumbnail?.source;
        if (src) {
          const url = `https://wsrv.nl/?url=${encodeURIComponent(src)}&w=350&h=350&fit=cover`;
          wikipediaImageCacheRef.current[playerName] = url;
          imgEl.src = url;
        } else {
          imgEl.src = FALLBACK_PLAYER_IMAGE;
        }
      })
      .catch(() => { imgEl.src = FALLBACK_PLAYER_IMAGE; });
  }, []);
  ...
  const [soccerSeasonYear, setSoccerSeasonYear] = useState(null);
  const f1Data = useF1Data(selectedSport, currentTab);
  ...
  // Pre-fetch Wikipedia thumbnails for cricket players without Commons/ESPN override (fixes ~23 missing images)
  useEffect(() => {
    if (selectedSport !== 'cricket') return;
    const list = (EXTRA_SPORT_PLAYERS.cricket || []).filter(p => !CRICKET_HEADSHOT_OVERRIDES[p.id]);
    if (list.length === 0) return;
    // Batch requests (10 at a time) to avoid rate limits; use thumbnail or originalimage
    const BATCH = 10;
    const fetchOne = (p) => {
      const name = CRICKET_WIKIPEDIA_FALLBACK_NAMES[p.id] || p.name;
      const slug = name.replace(/\s+/g, ' ').trim().replace(/ /g, '_');
      return fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(slug)}`)
        .then(r => r.ok ? r.json() : {})
        .then(d => ({ id: p.id, src: d.thumbnail?.source || d.originalimage?.source }));
    };
    const runBatch = (offset) => {
      const chunk = list.slice(offset, offset + BATCH);
      if (chunk.length === 0) return Promise.resolve();
      return Promise.all(chunk.map(fetchOne)).then(results => {
        const next = {};
        results.forEach(r => { if (r.src) next[r.id] = WSRV + encodeURIComponent(r.src) + '&w=350&h=350&fit=cover'; });
        setCricketWikiUrls(prev => ({ ...prev, ...next }));
        return offset + BATCH < list.length ? runBatch(offset + BATCH) : null;
      });
    };
    runBatch(0).catch(() => {});
  }, [selectedSport]);
  ...
  // Persistence: Supabase when logged in (real-time); localStorage for guests only
  useEffect(() => {
    if (user?.uid) {
      setUserData(user.uid, { bookedTickets }).catch((e) => console.error('Save error:', e));
    } else {
  ...
  useEffect(() => {
    if (user?.uid) {
      setUserData(user.uid, { favoriteClubs: favorites, favoritePlayers }).catch((e) => console.error('Save error:', e));
    } else {
  ...
  useEffect(() => {
    if (user?.uid) {
      setUserData(user.uid, { penaltyBest }).catch((e) => console.error('Save error:', e));
    } else {
  ...
  useEffect(() => {
    if (user?.uid) {
      setUserData(user.uid, { superOverBest }).catch((e) => console.error('Save error:', e));
    } else {
  ...
  const fetchMatchDetails = async (match) => {
    ...
    try {
      ...
      try {
        ...
        if (selectedSport === 'cricket' && enriched.gameLeaders.length === 0 && data.boxscore?.teams) {
          ...
          data.boxscore.teams.forEach(t => {
            const teamAbbr = t.team?.abbreviation || t.team?.displayName || '';
            (t.statistics || []).forEach(s => {
              ...
              if (statName.includes('batting') || statName.includes('runs') || statName === 'r') {
                (s.athletes || []).forEach(a => {
                  const stat = a.statistics?.[0] || a;
                  const runs = stat.runs ?? stat.R ?? stat.displayValue;
                  const name = a.displayName || a.athlete?.displayName;
                  if (name != null) allBatters.push({
                    displayName: name,
                    team: teamAbbr,
                    value: runs != null ? (typeof runs === 'number' ? `${runs} runs` : String(runs)) : (stat.displayValue || '—'),
                    headshot: a.athlete?.headshot?.href || a.athlete?.headshot || ''
                  });
                });
              }
              if (statName.includes('bowling') || statName.includes('wicket') || statName === 'w' || statName.includes('overs')) {
                (s.athletes || []).forEach(a => {
                  const stat = a.statistics?.[0] || a;
                  const wickets = stat.wickets ?? stat.W ?? stat.displayValue;
                  const name = a.displayName || a.athlete?.displayName;
                  if (name != null) allBowlers.push({
                    displayName: name,
                    team: teamAbbr,
                    value: wickets != null ? (typeof wickets === 'number' ? `${wickets} wkts` : String(wickets)) : (stat.displayValue || '—'),
                    headshot: a.athlete?.headshot?.href || a.athlete?.headshot || ''
                  });
                });
              }
          ...
          const topWickets = allBowlers
            ...
            .slice(0, 5);
          if (topRuns.length || topWickets.length) {
            enriched.gameLeaders = [];
            if (topRuns.length) enriched.gameLeaders.push({ displayName: 'Top Run Scorers', leaders: topRuns });
            if (topWickets.length) enriched.gameLeaders.push({ displayName: 'Top Wicket Takers', leaders: topWickets });
          }
        }
      ...
      try {
        ...
        // Cricket: build key events (live updates) from summary notes - every milestone, wicket, session
        if (selectedSport === 'cricket' && Array.isArray(data.notes) && data.notes.length > 0) {
          enriched.keyEvents = data.notes.map((note, i) => {
            const text = note.text || '';
            const typeStr = note.type || 'matchnote';
            const section = note.section != null ? `Innings ${note.section}` : '';
            const shortDisplay = text.length > 40 ? text.slice(0, 37) + '…' : text;
            return {
              id: note.id || i,
              type: { text: typeStr },
              text,
              shortText: shortDisplay,
              clock: section ? { displayValue: section } : { displayValue: typeStr.replace(/_/g, ' ') },
              participants: note.athlete ? [{ athlete: { displayName: note.athlete.displayName || note.athlete.fullName } }] : undefined
            };
          });
        }
        // Cricket: if no notes, try to build key events from boxscore (batting/bowling highlights)
        if (selectedSport === 'cricket' && (!enriched.keyEvents || enriched.keyEvents.length === 0) && data.boxscore?.teams?.length >= 2) {
          const events = [];
          data.boxscore.teams.forEach((t, teamIdx) => {
            const teamName = t.team?.abbreviation || t.team?.displayName || '';
            (t.statistics || []).forEach(stat => {
              const name = (stat.name || '').toLowerCase();
              if (name.includes('batting') || name.includes('runs') || name === 'r') {
                (stat.athletes || []).slice(0, 11).forEach((a, i) => {
                  const disp = a.displayName || a.athlete?.displayName;
                  const val = a.statistics?.[0]?.displayValue ?? a.displayValue ?? a.runs ?? a.R;
                  if (disp && val != null) events.push({ type: { text: 'Run' }, text: `${disp} ${val} runs`, shortText: `${val}`, clock: { displayValue: teamName }, participants: [{ athlete: { displayName: disp } }] });
                });
              }
              if (name.includes('bowling') || name.includes('wicket') || name === 'w') {
                (stat.athletes || []).slice(0, 6).forEach((a, i) => {
                  const disp = a.displayName || a.athlete?.displayName;
                  const val = a.statistics?.[0]?.displayValue ?? a.displayValue ?? a.wickets ?? a.W;
                  if (disp && val != null) events.push({ type: { text: 'Wicket' }, text: `${disp} ${val} wicket(s)`, shortText: `${val} W`, clock: { displayValue: teamName }, participants: [{ athlete: { displayName: disp } }] });
                });
              }
            });
          });
          if (events.length > 0) enriched.keyEvents = events;
        }
      } catch (e) { enriched.scoringPlays = []; if (selectedSport === 'cricket') enriched.keyEvents = enriched.keyEvents || []; }
      ...
      // --- Man of the Match (cricket and others) ---
      try {
        const awards = data.awards || [];
        const momAward = awards.find(a => /man of the match|player of the match|mom|potm/i.test(a.displayName || a.name || ''));
        if (momAward?.athlete) {
          enriched.manOfTheMatch = {
            name: momAward.athlete.displayName || momAward.athlete.fullName || '',
            team: momAward.team?.abbreviation || momAward.team?.displayName || '',
            value: momAward.displayValue || momAward.statistics?.[0]?.displayValue || ''
          };
        } else if (selectedSport === 'cricket' && data.leaders?.length > 0) {
          const firstCat = data.leaders[0];
          const firstLeader = firstCat?.leaders?.[0];
          if (firstLeader?.athlete) {
            enriched.manOfTheMatch = {
              name: firstLeader.athlete.displayName || firstLeader.athlete.fullName || '',
              team: firstLeader.team?.abbreviation || firstLeader.athlete?.team?.abbreviation || '',
              value: firstLeader.displayValue || firstLeader.statistics?.[0]?.displayValue || ''
            };
          }
        }
      } catch (e) { /* ignore */ }

      // --- Cricket Scorecard (for Scorecard tab: batting, bowling, extras, FOW, powerplays, partnerships) ---
      try {
        if (selectedSport === 'cricket' && data.boxscore?.teams?.length >= 2) {
          const periodScoresForSc = enriched.periodScores || [];
          enriched.cricketScorecard = {
            innings: data.boxscore.teams.map((t, idx) => {
              const battingStat = t.statistics?.find(s => /batting|runs|r\b/i.test(s.name || '') && s.athletes?.length);
              const otherTeam = data.boxscore.teams[(idx + 1) % 2];
              const bowlingStat = otherTeam?.statistics?.find(s => /bowling|wicket|w\b|overs/i.test(s.name || '') && s.athletes?.length);
              const batting = (battingStat?.athletes || []).map(a => {
                const stat = a.statistics?.[0] || a;
                return {
                  name: a.displayName || a.athlete?.displayName || '—',
                  runs: stat.runs ?? stat.R ?? stat.displayValue,
                  balls: stat.balls ?? stat.B,
                  fours: stat.fours ?? stat['4s'],
                  sixes: stat.sixes ?? stat['6s'],
                  sr: stat.strikeRate ?? stat.SR,
                  dismissal: a.dismissal?.text || a.dismissal || stat.dismissal || (a.notOut ? 'not out' : '—')
                };
              });
              const bowling = (bowlingStat?.athletes || []).map(a => {
                const stat = a.statistics?.[0] || a;
                return {
                  name: a.displayName || a.athlete?.displayName || '—',
                  o: stat.overs ?? stat.O,
                  m: stat.maidens ?? stat.M,
                  r: stat.runs ?? stat.R,
                  w: stat.wickets ?? stat.W,
                  nb: stat.noBalls ?? stat.NB,
                  wd: stat.wides ?? stat.WD,
                  eco: stat.economy ?? stat.ECO
                };
              });
              const ps = periodScoresForSc[idx];
              const totalScore = ps?.totalScore || t.statistics?.find(s => /total|runs/i.test(s.name || ''))?.displayValue || '—';
              const overs = ps?.linescores?.[0] != null ? String(ps.linescores[0]) : null;
              const runRate = overs && totalScore ? (parseFloat(String(totalScore).replace(/\D/g, '')) / parseFloat(overs)).toFixed(2) : null;
              const fallOfWickets = [];
              const powerplays = [];
              const partnerships = [];
              if (Array.isArray(data.notes)) {
                data.notes.filter(n => n.section === String(idx + 1) || n.section === String(idx)).forEach(note => {
                  const text = (note.text || '').trim();
                  const type = (note.type || '').toLowerCase();
                  if (type.includes('fall') || /\d+-\d+/.test(text)) {
                    const match = text.match(/(\d+-\d+)\s*[,\s]*(\d+\.?\d*)/) || text.match(/(\d+-\d+)/);
                    if (match) fallOfWickets.push({ batsman: text.split(/\d+-\d+/)[0].trim() || note.athlete?.displayName || '—', score: match[1], over: match[2] || '—' });
                  }
                  if (type.includes('powerplay') && text) powerplays.push({ type: text.split(/[0-9.]/)[0].trim() || 'Powerplay', overs: text.match(/\d+\.?\d*\s*[-–]\s*\d+/)?.[0] || '—', runs: text.match(/\d+\s*runs?/)?.[0] || '—' });
                  if (type.includes('partner') && text) partnerships.push({ text });
                });
              }
              return {
                team: t.team?.abbreviation || t.team?.displayName || '',
                totalScore,
                overs,
                runRate,
                batting,
                bowling,
                extras: t.extras != null ? t.extras : null,
                didNotBat: Array.isArray(t.didNotBat) ? t.didNotBat : [],
                fallOfWickets: fallOfWickets.length ? fallOfWickets : null,
                powerplays: powerplays.length ? powerplays : null,
                partnerships: partnerships.length ? partnerships : null
              };
            })
          };
        }
      } catch (e) { if (selectedSport === 'cricket') enriched.cricketScorecard = null; }
  ...
  // Poll live match details (cricket, football, etc.) so Key Plays / updates stay current while modal is open
  useEffect(() => {
    if (!selectedMatchStatus?.isLive || !selectedMatchStatus?.id) return;
    const interval = setInterval(() => {
      fetchMatchDetails(selectedMatchStatus);
    }, 45000);
    return () => clearInterval(interval);
  }, [selectedMatchStatus?.id, selectedMatchStatus?.isLive]);
  ...
  useEffect(() => {
    ...
    const transferCategoryPattern = /transfer\s*talk|transfer\s*rumor|transfer\s*rumour|transfer\s*news|transfers|transfer\s*market|signing\s*news|rumors?\s*&\s*rumours?|done\s*deal|transfer\s*centre|free\s*agency|trade\s*rumor|trade\s*news|signing|trades?|contract|waived|acquired|traded\s+to|roster\s*move|extension\s*talk|auction|retention|released\s*players|draft\b|trade\s*window|re-signed|moves?\s*news|ins\s*&\s*outs|transfer\s*round-up|silly\s*season/i;
    const transferHeadlinePattern = /transfer\s*(rumors?|news|talk|round|round-up|centre)\s*[,:]?|transfer\s*rumors?,?\s*news|signing|signed\s+for|joins\s+\w+|agrees\s+deal|trade\s*(rumors?|news|deadline)|free\s*agency|signed\s+with|contract\s+extension|waived\s+by|traded\s+to|acquired\s+by|ipl\s*auction|bbl\s*draft|retained\s+by|released\s+by|re-signs?\b|done\s*deal|move\s+to\s+\w+|driver\s+(move|switch|signs?|joins?|confirmed)|team\s+switch|f1\s+transfer|line-?up\s+news|seat\s+at\s+\w+|2026\s+drive|2025\s+drive|new\s+contract|extension\s+with|free\s*agent\s+signing|roster\s*move|squad\s+update|auction\s+result|retention\s+list|sold\s+to\s+\w+|bought\s+by\s+\w+/i;
    const transferDescriptionPattern = /\b(transfer|signing|signed|joins?|move[sd]?\s+to|deal\s+with|contract\s+extension|free\s*agency|traded|waived|acquired|trade\s*deadline|auction|retained|released|re-signed|driver\s+(move|switch|signs?|joins?)|team\s+switch|seat\s+at|line-?up|2026\s+drive|rumour|rumor|done\s*deal|extension\s+talk|roster\s*move|squad\s+announcement|retention\s+list)\b/i;
    ...
    const fetchAllSportFeeds = async () => {
      try {
          ...
          Object.entries(config.leagues).forEach(([leagueKey, code]) => {
            ...
            allRequests.push(
              fetch(`${base}/${path}/${code}/news?limit=${NEWS_LIMIT_PER_LEAGUE}`)
                .then((r) => r.json())
                .then((data) => {
                  const articles = Array.isArray(data?.articles) ? data.articles : (Array.isArray(data?.headlines) ? data.headlines : (Array.isArray(data) ? data : []));
                  return { data: articles, source, sportKey };
                })
        ...
        results.forEach(({ data: articles, source, sportKey }) => {
          (articles || []).forEach((a) => {
            const categoryDesc = (a.categories?.[0]?.description || a.category || '').trim();
            const headline = (a.headline || a.title || '').trim();
            const headlineLower = headline.toLowerCase();
            const desc = (a.description || a.summary || a.excerpt || '').toLowerCase();
            const item = {
              tag: fixTextEncoding(a.categories?.[0]?.description || a.category) || source,
              title: fixTextEncoding(headline || a.title) || '',
              excerpt: fixTextEncoding(a.description || a.summary || a.excerpt) || '',
              image: a.images?.[0]?.url || a.image?.url || a.thumbnail || 'https://via.placeholder.com/400x200',
              link: a.links?.web?.href || a.url || a.link,
              source,
              sportKey,
              published: a.published || a.pubDate || a.lastModified || a.date || new Date().toISOString()
            };

            const isTransfer = transferCategoryPattern.test(categoryDesc) || transferHeadlinePattern.test(headline) || transferDescriptionPattern.test(desc);
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      // F1: one card per Grand Prix (deduplicate by event id – API can return same weekend multiple times)
      if (selectedSport === 'f1' && allMatches.length > 0) {
        const seen = new Set();
  ...
  const fetchTable = useCallback(async (key, forceRefresh = false, seasonYearParam = null) => {
    ...
    const requestedSeason = isCricket ? seasonYearParam : (isSoccer ? seasonYearParam : null);
    const cacheKey = key;
    // For cricket/soccer: when requesting a specific season, always fetch if cached season doesn't match (or force refresh)
    const cached = tables[cacheKey];
    const seasonMismatch = (isCricket || isSoccer) && requestedSeason != null && cached?.seasonYear != null && cached.seasonYear !== requestedSeason;
    ...
    try {
      ...
      // Soccer: when a specific season is selected, try fallback first (historical data since 2008–09)
      if (isSoccer && requestedSeason != null && FOOTBALL_LEAGUES_WITH_HISTORY.includes(key)) {
        const fallback = getFootballStandingsFallback(key, requestedSeason);
        if (fallback?.conferences?.length > 0) {
          setTables((prev) => ({
            ...prev,
            [key]: {
      ...
      const seasonQuery = (isCricket || isSoccer) && requestedSeason != null ? `?season=${requestedSeason}` : '';
      ...
      // FIFA World Cup: multiple children = groups (Group A, B, C, ...)
      if (selectedSport === 'soccer' && key === 'worldcup' && children.length > 1) {
        const conferences = children.map(parseConference).filter(Boolean);
  ...
  const fetchWorldCupKnockoutMatches = useCallback(async () => {
    if (selectedSport !== 'soccer') return;
    try {
      // FIFA World Cup: typically June–July (e.g. 2026: Jun 11 – Jul 19)
      const year = new Date().getFullYear();
      const start = new Date(year, 5, 1);   // June 1
      const end = new Date(year, 6, 31);    // July 31
      const datesParam = `${start.toISOString().slice(0, 10).replace(/-/g, '')}-${end.toISOString().slice(0, 10).replace(/-/g, '')}`;
      const res = await fetch(`${apiBase}/fifa.world/scoreboard?dates=${datesParam}&limit=500`);
      const data = await res.json();
      const events = data.events || [];
      const knockoutKeywords = /Round of 32|Round of 16|Quarterfinal|Semifinal|Semi-Final|Third-Place|Final|Knockout/i;
      const wcMatches = events.filter(ev => {
        const comp = ev.competitions?.[0];
        const desc = (comp?.description || '') + (comp?.shortDescription || '') + (comp?.notes?.[0]?.text || '');
        return comp && (knockoutKeywords.test(desc) || ev.type?.slug === 'knockout');
      }).map(event => {
        const comp = event.competitions[0];
        ...
        const dateStr = matchDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
        const timeStr = matchDate.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' });
        const roundLabel = comp?.description || comp?.shortDescription || comp?.notes?.[0]?.text || '';
        return {
          id: event.id,
      ...
      setWorldCupKnockoutMatches(wcMatches);
    } catch (e) {
      console.error('World Cup knockout fetch error:', e);
    }
  }, [apiBase, selectedSport]);
  ...
  const fetchCricketKnockoutMatches = useCallback(async (leagueKey, code, seasonYearParam = null) => {
    ...
    try {
      ...
      const datesParam = `${start.toISOString().slice(0, 10).replace(/-/g, '')}-${end.toISOString().slice(0, 10).replace(/-/g, '')}`;
      let res = await fetch(`${apiBase}/${code}/scoreboard?dates=${datesParam}&limit=500`);
      let data = await res.json().catch(() => ({}));
      let events = data.events || [];
      // T20 WC: if scoreboard returns no or few events, try without date filter (current tournament)
      if (leagueKey === 't20wc' && events.length < 10) {
        try {
          const calRes = await fetch(`${apiBase}/${code}/scoreboard?limit=500`);
          const calData = await calRes.json().catch(() => ({}));
          const calEvents = calData.events || [];
          if (calEvents.length > events.length) events = calEvents;
        } catch (_) {}
      }
      // T20 WC: also try with season param and merge so we get all phases (group + semi + final)
      if (leagueKey === 't20wc') {
        try {
          const seasonRes = await fetch(`${apiBase}/${code}/scoreboard?season=${year}&limit=500`);
          const seasonData = await seasonRes.json().catch(() => ({}));
          const seasonEvents = seasonData.events || [];
          const byId = new Map(events.map(e => [e.id, e]));
          seasonEvents.forEach(e => { if (e.id && !byId.has(e.id)) byId.set(e.id, e); });
          if (byId.size > events.length) events = Array.from(byId.values());
        } catch (_) {}
      }
      ...
      const knockoutKeywords = /Final|Semi|Qualifier|Eliminator|Playoff|playoff|knockout|Quarter|SF1|SF2|semi final|grand final|1st semi|2nd semi|first semi|second semi|semifinal/i;
      const getSearchableText = (ev) => {
        const comp = ev.competitions?.[0];
        return [
          comp?.description,
          ...
          ev.season?.slug,
          ev.season?.year,
          ...(comp?.notes || []).map(n => n.text).filter(Boolean)
        ].filter(Boolean).join(' ');
      };
      let knockoutEvents = events.filter(ev => {
        const comp = ev.competitions?.[0];
        const text = getSearchableText(ev);
        return comp && knockoutKeywords.test(text);
      });
      // Exclude group stage, Super Eights, and any "Nth MATCH" — then keep only Semi-Final 1, Semi-Final 2, Final
      const groupStagePattern = /\d+(st|nd|rd|th)\s*MATCH|,\s*GROUP\s*[A-Z]|GROUP\s+[A-Z]|SUPER\s*EIGHTS/i;
      knockoutEvents = knockoutEvents.filter(ev => !groupStagePattern.test(getSearchableText(ev)));
      const getRoundOnlyText = (ev) => {
        const comp = ev.competitions?.[0];
        return [
          comp?.description,
          comp?.shortDescription,
      ...
      const isSemiOrFinal = (text) => {
        const t = (text || '').toLowerCase();
        if (/semi\s*final\s*1|1st\s*semi|sf1|semi\s*1/.test(t)) return true;
        if (/semi\s*final\s*2|2nd\s*semi|sf2|semi\s*2/.test(t)) return true;
        if (/\bfinal\b/.test(t) && !/semi|quarter|group|super|\d+(st|nd|rd|th)\s*match|match\s*\d+/i.test(t)) return true;
        return false;
      };
      knockoutEvents = knockoutEvents.filter(ev => isSemiOrFinal(getRoundOnlyText(ev)));
      ...
      const mapped = knockoutEvents.map(event => {
        ...
        const timeStr = matchDate.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' });
        const roundLabel = comp?.description || comp?.shortDescription || comp?.name || event.name || event.shortName || '';
  ...
  const fetchPlayerLiveStats = useCallback(async () => {
    // Fetch live goals/assists (current season) from ESPN so stats are as-of-today and increase as players score.
    if (selectedSport !== 'soccer') return;
    const base = `${SPORTS_API_SITE_ROOT}/soccer`;
    const soccerMap = {};
    const nameToOurId = Object.fromEntries(PLAYERS_DATA.map((p) => [p.name.toLowerCase().trim(), p.id]));
    for (const code of SOCCER_STATS_LEAGUES) {
      try {
        const res = await fetch(`${base}/${code}/statistics`);
        const data = await res.json();
        ...
        for (const stat of statsList) {
          ...
          for (const leader of leaders) {
            ...
            let ourId = SOCCER_ESPN_ID_TO_OUR_ID[espnId] || nameToOurId[displayName.toLowerCase()];
            if (!ourId && displayName) {
              const normalized = displayName.toLowerCase().replace(/\s+/g, ' ').normalize('NFD').replace(/\p{Diacritic}/gu, '');
              ourId = PLAYERS_DATA.find((p) => p.name.toLowerCase().replace(/\s+/g, ' ').normalize('NFD').replace(/\p{Diacritic}/gu, '').includes(normalized) || normalized.includes(p.name.toLowerCase().replace(/\s+/g, ' ').normalize('NFD').replace(/\p{Diacritic}/gu, '')))?.id;
            }
            if (!ourId) continue;
            const statsArr = leader.statistics || [];
            const totalGoals = statsArr.find((s) => s.name === 'totalGoals' || s.abbreviation === 'G');
            const goalAssists = statsArr.find((s) => s.name === 'goalAssists' || s.abbreviation === 'A');
            const g = leader.value != null ? Number(leader.value) : (totalGoals?.displayValue != null ? Number(totalGoals.displayValue) : totalGoals?.value != null ? Number(totalGoals.value) : null);
            const a = goalAssists != null ? (goalAssists.displayValue != null ? Number(goalAssists.displayValue) : goalAssists.value != null ? Number(goalAssists.value) : null) : null;
            if (g != null || a != null) {
              const prev = soccerMap[ourId] || {};
              soccerMap[ourId] = { ...prev, ...(g != null && { goals: g }), ...(a != null && { assists: a }) };
            }
          }
        }
      } catch (_e) { /* league may not support statistics */ }
    }
    setPlayerLiveStats((prev) => ({ ...prev, soccer: soccerMap }));
  }, [selectedSport]);
  ...
  useEffect(() => {
    const run = () => fetchSportPlayers().then(() => setLastPlayersRefreshAt(Date.now())).then(() => { if (selectedSport === 'soccer') fetchPlayerLiveStats(); });
    run();
    const interval = setInterval(run, LIVE_PLAYERS_REFRESH_INTERVAL_MS);
    return () => clearInterval(interval);
  }, [fetchSportPlayers, fetchPlayerLiveStats, selectedSport]);
  ...
  // "Updated Xs ago" ticker for Top Players: updates every second when on Players tab
  useEffect(() => {
    if (currentTab !== 'players') return;
    const tick = () => {
      if (lastPlayersRefreshAt != null) setSecondsSinceLastRefresh(Math.floor((Date.now() - lastPlayersRefreshAt) / 1000));
    };
    tick();
    const interval = setInterval(tick, 1000);
    return () => clearInterval(interval);
  }, [currentTab, lastPlayersRefreshAt]);

  useEffect(() => {
    if (leagueNames[currentTab]) {
      const seasonParam = selectedSport === 'cricket' ? cricketSeasonYear : (selectedSport === 'soccer' ? soccerSeasonYear : undefined);
      fetchTable(currentTab, false, seasonParam);
      const interval = setInterval(() => fetchTable(currentTab, true, selectedSport === 'cricket' ? cricketSeasonYear : (selectedSport === 'soccer' ? soccerSeasonYear : undefined)), 1000);
      return () => clearInterval(interval);
    }
  }, [currentTab, fetchTable, leagueNames, selectedSport, cricketSeasonYear, soccerSeasonYear]);
  ...
  // Reset soccer season when switching to a league that doesn't have that year
  useEffect(() => {
    if (selectedSport !== 'soccer' || soccerSeasonYear == null) return;
    const validYears = getFootballSeasonYears(currentTab);
    if (validYears.indexOf(soccerSeasonYear) === -1) setSoccerSeasonYear(null);
  }, [selectedSport, currentTab, soccerSeasonYear]);
  ...
  useEffect(() => {
    if (currentTab === 'worldcup' && worldcupTab === 'knockout') {
      fetchWorldCupKnockoutMatches();
    }
  }, [currentTab, worldcupTab, fetchWorldCupKnockoutMatches]);
  ...
  /** Merge live stats into player. For soccer: show career total = baseline (ESPN/official sources) + live season goals/assists from ESPN so it updates as they score. */
  const getDisplayPlayer = useCallback((p) => {
    const live = playerLiveStats[selectedSport];
    const patch = live ? (live[p.id] ?? live[String(p.id)]) : null;
    if (selectedSport === 'soccer') {
      const baseGoals = SOCCER_CAREER_BASELINE[p.id]?.goals ?? p.goals;
      const baseAssists = SOCCER_CAREER_BASELINE[p.id]?.assists ?? p.assists;
      if (patch) {
        return {
          ...p,
          goals: (baseGoals != null ? Number(baseGoals) : 0) + (patch.goals != null ? Number(patch.goals) : 0),
          assists: (baseAssists != null && baseAssists !== '-' ? Number(baseAssists) : 0) + (patch.assists != null ? Number(patch.assists) : 0)
        };
      }
      return p;
    }
    ...
    return p;
  }, [playerLiveStats, selectedSport]);

  /** Ensure every player has careerStations and statsByCompetition for the modal (fallback from career + totals). */
  const enrichPlayerForModal = useCallback((p) => {
    if (!p) return p;
    let careerStations = p.careerStations;
    if (!careerStations?.length && p.career?.length) {
      careerStations = p.career.map(team => ({ team, period: '–', goals: '–', assists: '–', apps: '–' }));
    }
    let statsByCompetition = p.statsByCompetition;
    if (!statsByCompetition || Object.keys(statsByCompetition).length === 0) {
      const g = p.goals != null ? p.goals : '–';
      const a = (p.assists != null && p.assists !== '') ? p.assists : '–';
      if (g !== '–' || a !== '–') statsByCompetition = { 'Career total': { goals: g, assists: a } };
    }
    return {
      ...p,
      careerStations: careerStations || [],
      statsByCompetition: statsByCompetition || {},
      trophies: Array.isArray(p.trophies) ? p.trophies : (p.trophies ? [p.trophies] : []),
      achievements: Array.isArray(p.achievements) ? p.achievements : (p.achievements ? [p.achievements] : [])
    };
  }, []);
  ...
  const renderSection = () => {
    switch (currentTab) {
      ...
      case 'worldcup':
        ...
        return (
          <section key={currentTab} className="content-section active">
            ...
            <div className={`date-navigator-container${showCalendar ? ' calendar-open' : ''}`}>
              ...
              <div className="date-trigger-btn" onClick={() => {
                if (!showCalendar) {
                  const [y, m, d] = selectedDate.split('-').map(Number);
                      ...
                      for (let i = 1; i <= daysInMonth; i++) {
                        ...
                        days.push(
                          <div
                            ...
                            onClick={(e) => { e.stopPropagation(); setSelectedDate(isoDate); setShowCalendar(false); }}
                            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelectedDate(isoDate); setShowCalendar(false); } }}
                          >
                            {i}
                          </div>
                        );
            ...
            {currentTab === 'minigames' && (selectedSport === 'basketball' || selectedSport === 'f1') ? (
              <>
                <div className="section-header-pro">
                  <h3>
                    <span className="material-icons-round section-header-sport-icon" aria-hidden="true">sports_esports</span>
                    Mini games
                  </h3>
                </div>
                <div className="fade-in" style={{ marginTop: '24px' }}>
                  {selectedSport === 'basketball' && <BasketballMiniGames />}
                  {selectedSport === 'f1' && <F1MiniGames />}
                </div>
              </>
            ) : selectedSport === 'f1' && currentTab === 'f1' ? (
              <>
                <div className="section-header-pro">
                  <h3>
                ...
                <F1Section
                  f1Tab={f1Tab}
                  setF1Tab={setF1Tab}
                  calendar={f1Data.calendar}
                  standings={f1Data.standings}
                  loading={f1Data.loading}
                  error={f1Data.error}
                  onRefresh={f1Data.refresh}
                      ...
                      <div className="matches-grid">
                        {filteredMatches.length > 0 ? (
                          filteredMatches
                            ...
                            .map(m => <MatchCard key={m.id} match={m} favorites={favorites} toggleFavorite={toggleFavorite} onOpen={fetchMatchDetails} showFavorite={featureFlags.favorites !== false} hideLeagueLabel />)
                        ) : (
                          <div style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: '20px', width: '100%' }}>
                            {search ? 'No matches found for this search.' : 'No races found for this date.'}
                          </div>
                        )}
                      </div>
                      <Pagination
              ...
              <div className="section-header-pro">
                ...
                {selectedSport === 'soccer' && currentTab === 'worldcup' && (
                  <div className="toggle-tabs">
                    <button className={`toggle-btn ${worldcupTab === 'groups' ? 'active' : ''}`} onClick={() => setWorldcupTab('groups')}>Group Stage</button>
                    <button className={`toggle-btn ${worldcupTab === 'matches' ? 'active' : ''}`} onClick={() => setWorldcupTab('matches')}>Matches</button>
                    <button className={`toggle-btn ${worldcupTab === 'knockout' ? 'active' : ''}`} onClick={() => setWorldcupTab('knockout')}>Knockout</button>
                  </div>
                )}
            ...
            {/* Champions League: Group Stage (historical seasons – same layout as World Cup groups) */}
            {currentTab === 'ucl' && uclTab === 'league' && tables['ucl']?.conferences?.length > 0 && (
              <div className="fade-in" style={{ marginTop: '24px' }}>
                <div className="league-header" style={{ marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                    <img loading="lazy" decoding="async" src={leagueLogos.ucl} alt="" className="league-brand-logo" onError={(e) => { e.target.src = FALLBACK_TEAM_LOGO; }} />
                    <h3 style={{ fontSize: '18px', fontWeight: 800 }}>
                      Champions League — Group Stage
                      {tables['ucl'].seasonYear && (
                        <span style={{ fontWeight: 600, color: 'var(--text-secondary, #94a3b8)', marginLeft: 6 }}>{(tables['ucl'].seasonYear - 1) + '–' + String(tables['ucl'].seasonYear).slice(-2)}</span>
                      )}
                    </h3>
                  </div>
                </div>
                {tables['ucl'].conferences.map((conf, ci) => (
                  <div key={ci} className="table-container" style={{ marginTop: ci === 0 ? 0 : '30px' }}>
                    <div className="league-header">
                      <h3 style={{ fontSize: '18px', fontWeight: 800 }}>{conf.name || `Group ${String.fromCharCode(65 + ci)}`}</h3>
                    </div>
                    <table className="standings-table">
                      ...
                      <tbody>
                        {(conf.rows || []).map((row, i) => (
                          <tr key={i} className={`animate-in ${getPositionClass('ucl', row.pos)}`} style={{ animationDelay: `${i * 0.03}s` }}>
            ...
            {/* FIFA World Cup: Group Stage tables (when Groups tab selected) */}
            {currentTab === 'worldcup' && worldcupTab === 'groups' && (
              <div className="fade-in" style={{ marginTop: '24px' }}>
                {tables['worldcup']?.conferences?.length > 0 ? (
                  tables['worldcup'].conferences.map((conf, ci) => (
                    <div key={ci} className="table-container" style={{ marginTop: ci === 0 ? 0 : '30px' }}>
                      <div className="league-header">
                        <h3 style={{ fontSize: '18px', fontWeight: 800 }}>{conf.name || `Group ${String.fromCharCode(65 + ci)}`}</h3>
                      </div>
                ...
                ) : tables['worldcup']?.rows?.length > 0 ? (
                  <div className="table-container">
                    <div className="league-header"><h3 style={{ fontSize: '18px', fontWeight: 800 }}>FIFA World Cup Standings</h3></div>
                    <table className="standings-table">
                      <thead>
                        <tr>
                          <th>Pos</th><th>Team</th>
                          {(tables['worldcup'].columns || []).map((col) => <th key={col}>{col}</th>)}
                        </tr>
                      </thead>
                      <tbody>
                        {tables['worldcup'].rows.map((row, i) => (
                          <tr key={i} className="animate-in">
                            <td className="pos-cell">{row.pos}</td>
                            <td style={{ display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 600 }}>
                              <img loading="lazy" decoding="async" src={row.logo} style={{ width: '24px', height: '24px' }} alt="" onError={(e) => { e.target.src = FALLBACK_TEAM_LOGO; }} />
                            ...
                            {(tables['worldcup'].columns || []).map((col) => (
                              <td key={`${row.team}-${col}`}>{row.values?.[col] ?? '-'}</td>
                            ))}
                          </tr>
                        ))}
                  ...
                  <div className="loader-container" style={{ padding: '40px 20px' }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)', marginTop: 12 }}>Loading group standings…</p>
                  </div>
                )}
              </div>
            )}
            ...
            {currentTab === 'worldcup' && worldcupTab === 'knockout' && (
              <div className="knockout-bracket fade-in" style={{ marginTop: '24px' }}>
                <div className="league-header">
                  <h3>FIFA World Cup — Knockout Stage</h3>
                </div>
                {isMobile && (
                ...
                <div className="bracket-container">
                  <div className="bracket-grid">
                    {WORLD_CUP_KNOCKOUT_BRACKET.map(({ round, matches }) => {
                      const roundKey = round.replace(/\s+/g, '-');
                      const roundMatches = worldCupKnockoutMatches.filter(m =>
                        m.round && (
                          (round === 'Round of 32' && (m.round.includes('32') || m.round.includes('Round of 32'))) ||
                          (round === 'Round of 16' && (m.round.includes('16') || m.round.includes('Round of 16'))) ||
                          (round === 'Quarterfinal' && m.round.toLowerCase().includes('quarter')) ||
                          (round === 'Semifinal' && m.round.toLowerCase().includes('semi') && !m.round.toLowerCase().includes('third')) ||
                          (round === 'Third-Place' && m.round.toLowerCase().includes('third')) ||
                          (round === 'Final' && m.round.toLowerCase().includes('final') && !m.round.toLowerCase().includes('quarter') && !m.round.toLowerCase().includes('semi') && !m.round.toLowerCase().includes('third'))
                        )
                      );
                      return (
                        <div key={roundKey} className="bracket-round">
                          <h4 className="round-title">{round.toUpperCase().replace(/-/g, ' ')}</h4>
                          <div className="round-matches">
                            {matches.map((slot, i) => {
                              const realMatch = roundMatches[i];
                              if (realMatch) {
                                return (
                                  <div key={realMatch.id} className={`bracket-match-card ${realMatch.isLive ? 'is-live' : ''}`} onClick={() => fetchMatchDetails(realMatch)}>
                                    <div className="match-meta">
                                      {realMatch.isLive ? <span className="live-tag">● LIVE</span> : <span className="time-tag">{realMatch.time}</span>}
                                    </div>
                                    <div className="team-row">
                                      <div className="team-info-mini">
                                        <img loading="lazy" decoding="async" src={realMatch.homeLogo} alt="" onError={(e) => { e.target.src = FALLBACK_TEAM_LOGO; }} />
                                        <span className={realMatch.winner === 'home' ? 'winner' : ''}>{realMatch.home}</span>
                                      </div>
                                      <span className="score-mini">{realMatch.homeScore}</span>
                                    </div>
                                    <div className="team-row">
                                      <div className="team-info-mini">
                                        <img loading="lazy" decoding="async" src={realMatch.awayLogo} alt="" onError={(e) => { e.target.src = FALLBACK_TEAM_LOGO; }} />
                                        <span className={realMatch.winner === 'away' ? 'winner' : ''}>{realMatch.away}</span>
                                      </div>
                                      <span className="score-mini">{realMatch.awayScore}</span>
                                    </div>
                                  </div>
                                );
                              }
                              return (
                                <div key={`${roundKey}-${i}`} className="bracket-match-card placeholder-card">
                                  <div className="placeholder-team" title={slot.home}>{slot.home}</div>
                                  <div className="placeholder-team" title={slot.away}>{slot.away}</div>
                                </div>
                              );
                            })}
                          </div>
            ...
            {(currentTab !== 'worldcup' || worldcupTab === 'matches') && !(selectedSport === 'f1' && currentTab === 'f1') && (
              <>
            <div className="matches-grid">
              {filteredMatches.length > 0 ? (
                filteredMatches
            ...
            {/* Football (soccer): season selector from 2008–09 (season end year 2009) through current+1 */}
            {selectedSport === 'soccer' && FOOTBALL_LEAGUES_WITH_HISTORY.includes(currentTab) && currentTab !== 'worldcup' && (currentTab !== 'ucl' || uclTab === 'league') && (
              <div className="cricket-season-selector">
                <div className="cricket-season-selector-inner">
                  <span className="cricket-season-selector-icon material-icons-round" aria-hidden="true">calendar_today</span>
                  <label htmlFor="soccer-season-select" className="cricket-season-selector-label">Season</label>
                  <select
                    id="soccer-season-select"
                    className="cricket-season-selector-select"
                    value={soccerSeasonYear ?? ''}
                    onChange={(e) => {
                      const val = e.target.value;
                      const year = val === '' ? null : parseInt(val, 10);
                      setSoccerSeasonYear(year);
                      fetchTable(currentTab, true, year);
                    }}
                    aria-label="Select football season"
                  >
                    <option value="">Current</option>
                    {getFootballSeasonYears(currentTab).map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                  <span className="cricket-season-selector-hint">
                    <span className="material-icons-round" style={{ fontSize: '14px', verticalAlign: 'middle', marginRight: 4 }}>refresh</span>
                    Live · refreshes every second
            ...
            {/* Conference-based standings: NBA shows one table per sub-tab; NFL/NHL/MLB show all (UCL group stage uses dedicated block above) */}
            {leagueNames[currentTab] && currentTab !== 'worldcup' && currentTab !== 'ucl' && (currentTab !== 'ucl' || uclTab === 'league') && (selectedSport !== 'cricket' || !CRICKET_KNOCKOUT_LEAGUES.includes(currentTab) || cricketTab === 'league') && tables[currentTab]?.conferences?.length > 0 && (
              (() => {
            ...
            {/* Single-conference standings (Soccer, Cricket, F1) – exclude F1 here; F1 shows standings only in its Standings tab */}
            {leagueNames[currentTab] && (selectedSport !== 'f1' || currentTab !== 'f1') && currentTab !== 'worldcup' && (currentTab !== 'ucl' || uclTab === 'league') && (selectedSport !== 'cricket' || !CRICKET_KNOCKOUT_LEAGUES.includes(currentTab) || cricketTab === 'league') && !tables[currentTab]?.conferences?.length && tables[currentTab]?.rows?.length > 0 && (
            ...
            {/* League table section with season but no rows yet (e.g. PSL before season starts) – exclude F1 */}
            {leagueNames[currentTab] && (selectedSport !== 'f1' || currentTab !== 'f1') && currentTab !== 'worldcup' && (currentTab !== 'ucl' || uclTab === 'league') && (selectedSport !== 'cricket' || !CRICKET_KNOCKOUT_LEAGUES.includes(currentTab) || cricketTab === 'league') && tables[currentTab] && !tables[currentTab]?.conferences?.length && !(tables[currentTab]?.rows?.length > 0) && tables[currentTab].seasonYear && (
                  ...
                  <div className="bracket-grid" key={`knockout-${currentTab}-${cricketSeasonYear ?? 'current'}`}>
                    {(() => {
                      ...
                      const normalizeRound = (r) => {
                        ...
                        if (/semi.*1|1st semi|sf1/.test(s)) return 'Semi-Final 1';
                        if (/semi.*2|2nd semi|sf2/.test(s)) return 'Semi-Final 2';
                        if (/semi/.test(s)) return 'Semi-Final';
                        if (/final/.test(s) && !/semi|quarter|third/.test(s)) return 'Final';
                        if (/qualifier 1|qualifier1/.test(s)) return 'Qualifier 1';
                        if (/eliminator/.test(s)) return 'Eliminator';
                        if (/qualifier 2|qualifier2/.test(s)) return 'Qualifier 2';
                        if (/quarter/.test(s)) return 'Quarter-Final';
                        return r;
                      };
                      list.forEach(m => {
                        const r = normalizeRound(m.round || 'Other');
                        if (!byRound[r]) byRound[r] = [];
                        byRound[r].push(m);
                      });
                      const order = ['Qualifier 1', 'Eliminator', 'Qualifier 2', 'Semi-Final 1', 'Semi-Final 2', 'Semi-Final', 'Quarter-Final', 'Final'];
                      // If we have Final but no semi sections, add placeholder semi rounds so they're visible
                      const hasFinal = byRound['Final']?.length > 0;
                      const hasSemi1 = byRound['Semi-Final 1']?.length > 0;
                      const hasSemi2 = byRound['Semi-Final 2']?.length > 0;
                      if (hasFinal && (!hasSemi1 || !hasSemi2)) {
                        if (!byRound['Semi-Final 1']) byRound['Semi-Final 1'] = [];
                        if (!byRound['Semi-Final 2']) byRound['Semi-Final 2'] = [];
                        if (!hasSemi1) byRound['Semi-Final 1'].push({ id: 'placeholder-sf1', round: 'Semi-Final 1', home: 'TBD', away: 'TBD', homeScore: '–', awayScore: '–', time: 'TBC', isLive: false, isCompleted: false, homeLogo: FALLBACK_TEAM_LOGO, awayLogo: FALLBACK_TEAM_LOGO, placeholder: true });
                        if (!hasSemi2) byRound['Semi-Final 2'].push({ id: 'placeholder-sf2', round: 'Semi-Final 2', home: 'TBD', away: 'TBD', homeScore: '–', awayScore: '–', time: 'TBC', isLive: false, isCompleted: false, homeLogo: FALLBACK_TEAM_LOGO, awayLogo: FALLBACK_TEAM_LOGO, placeholder: true });
                      }
                      // Order rounds explicitly: Semi-Final 1, then Semi-Final 2, then Final (and any others by order array)
                      const roundOrder = ['Semi-Final 1', 'Semi-Final 2', 'Final', 'Qualifier 1', 'Eliminator', 'Qualifier 2', 'Semi-Final', 'Quarter-Final'];
                      const sortedRounds = roundOrder.filter(round => byRound[round]?.length > 0);
                      const remainingRounds = Object.keys(byRound).filter(r => !sortedRounds.includes(r));
                      remainingRounds.sort((a, b) => {
                        const ai = order.findIndex(o => (a || '').toLowerCase().includes(o.toLowerCase().replace(/-/g, ' ')));
                        const bi = order.findIndex(o => (b || '').toLowerCase().includes(o.toLowerCase().replace(/-/g, ' ')));
                        if (ai !== -1 && bi !== -1) return ai - bi;
                        if (ai !== -1) return -1;
                        if (bi !== -1) return 1;
                        return (a || '').localeCompare(b || '');
                      });
                      sortedRounds.push(...remainingRounds);
                        ...
                        <div key={roundName} className="bracket-round">
                          ...
                          <div className="round-matches">
                            {(byRound[roundName] || []).map((match, i) => (
                              <div key={match.id || i} className={`bracket-match-card ${match.isLive ? 'is-live' : ''} ${match.placeholder ? 'placeholder-card' : ''}`} onClick={() => !match.placeholder && fetchMatchDetails(match)}>
                                <div className="match-meta">
                                  {match.isLive ? <span className="live-tag">● LIVE</span> : <span className="time-tag">{match.time}</span>}
                                </div>
                                <div className="team-row">
                                ...
                                {match.isCompleted && match.statusDetail && !match.placeholder && <div className="match-status-mini">{match.statusDetail}</div>}
                                {match.placeholder && <div className="match-status-mini" style={{ color: 'var(--text-muted)' }}>From ESPN when available</div>}
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          <section key="players" className="content-section active players-tab-section">
            <div className="section-header-pro players-section-header">
              <h3>
                ...
                <span className="players-live-badge" aria-label="Live data">
                  <span className="live-dot" /> Live
                  {lastPlayersRefreshAt != null && (
                    <span className="players-updated-ago">
                      {' • '}
                      {secondsSinceLastRefresh < 5 ? 'Just now' : `Updated ${secondsSinceLastRefresh}s ago`}
                    </span>
                  )}
                </span>
              </h3>
              ...
              {filteredPlayers.length === 0 ? (
                ...
                <div className="players-grid">
                  {filteredPlayers
                    ...
                    .map(p => {
                      const d = getDisplayPlayer(p);
                      return (
                      <div key={String(p.id)} className="player-card" onClick={() => setSelectedPlayer(enrichPlayerForModal(getDisplayPlayer(p)))}>
                        <div className="player-image-container">
                          <img
                            loading="lazy"
                            decoding="async"
                            src={selectedSport === 'soccer' ? (PLAYER_HEADSHOT_OVERRIDES[p.id] || getHeadshot(p.id)) : selectedSport === 'cricket' ? (CRICKET_HEADSHOT_OVERRIDES[p.id] || cricketWikiUrls[p.id] || p.image) : p.image}
                            className="player-image"
                            alt={p.name}
                            referrerPolicy="no-referrer"
                            onError={(e) => {
                              e.target.onerror = null;
                              if (selectedSport === 'soccer' || selectedSport === 'cricket') setWikipediaFallbackImage((selectedSport === 'cricket' && CRICKET_WIKIPEDIA_FALLBACK_NAMES[p.id]) ? CRICKET_WIKIPEDIA_FALLBACK_NAMES[p.id] : p.name, e.target);
                              else e.target.src = FALLBACK_PLAYER_IMAGE;
                            }}
                          />
                        </div>
                        <div className="player-stats-mini">
                          ...
                          <div className="stats-grid">
                            <div className="stat-item"><span className="stat-val">{d.rating}</span><span className="stat-lbl">OVR</span></div>
                            <div className="stat-item"><span className="stat-val">{d.goals ?? '-'}</span><span className="stat-lbl">{d.primaryStatLabel || 'G'}</span></div>
                            <div className="stat-item"><span className="stat-val">{d.assists ?? '-'}</span><span className="stat-lbl">{d.secondaryStatLabel || 'A'}</span></div>
                          </div>
                        </div>
                      </div>
                    ); })}
              ...
              <div className="players-grid">
                {allPlayersIndex.filter(p => favoritePlayers.includes(p.id)).length > 0 ? (
                  allPlayersIndex.filter(p => favoritePlayers.includes(p.id))
                    ...
                    .map(p => (
                      <div key={p.id} className="player-card animate-in" onClick={() => setSelectedPlayer(enrichPlayerForModal(getDisplayPlayer(p)))}>
                        <div className="player-image-container">
                          <img loading="lazy" decoding="async" referrerPolicy="no-referrer" src={selectedSport === 'soccer' ? (PLAYER_HEADSHOT_OVERRIDES[p.id] || getHeadshot(p.id)) : selectedSport === 'cricket' ? (CRICKET_HEADSHOT_OVERRIDES[p.id] || cricketWikiUrls[p.id] || p.image) : p.image} className="player-image" alt={p.name} onError={(e) => { e.target.onerror = null; if (selectedSport === 'soccer' || selectedSport === 'cricket') setWikipediaFallbackImage((selectedSport === 'cricket' && CRICKET_WIKIPEDIA_FALLBACK_NAMES[p.id]) ? CRICKET_WIKIPEDIA_FALLBACK_NAMES[p.id] : p.name, e.target); else e.target.src = FALLBACK_PLAYER_IMAGE; }} />
                        </div>
                        <div className="player-stats-mini">
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div className="player-name">{p.name}</div>
                            <span className="material-icons-round fav-star active" onClick={(e) => { e.stopPropagation(); toggleFavoritePlayer(p.id); }}>star</span>
                          </div>
      ...
      case 'minigames':
        if (selectedSport === 'basketball' || selectedSport === 'f1') {
          return (
            <section key="minigames" className="content-section active">
              <div className="section-header-pro">
                <h3>
                  <span className="material-icons-round section-header-sport-icon" aria-hidden="true">sports_esports</span>
                  Mini games
    ...
    <>
      ...
      {celebration && (
          ...
          <div className="modal-overlay" onClick={() => setSelectedPlayer(null)}>
            <div className="modal-content animate-in player-detail-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-hero">
                <img loading="lazy" decoding="async" referrerPolicy="no-referrer" src={selectedSport === 'soccer' ? (PLAYER_HEADSHOT_OVERRIDES[selectedPlayer.id] || getHeadshot(selectedPlayer.id)) : selectedSport === 'cricket' ? (CRICKET_HEADSHOT_OVERRIDES[selectedPlayer.id] || cricketWikiUrls[selectedPlayer.id] || selectedPlayer.image) : selectedPlayer.image} className="modal-player-img" alt="" onError={(e) => { e.target.onerror = null; if (selectedSport === 'soccer' || selectedSport === 'cricket') setWikipediaFallbackImage((selectedSport === 'cricket' && CRICKET_WIKIPEDIA_FALLBACK_NAMES[selectedPlayer.id]) ? CRICKET_WIKIPEDIA_FALLBACK_NAMES[selectedPlayer.id] : selectedPlayer.name, e.target); else e.target.src = FALLBACK_PLAYER_IMAGE; }} />
              ...
              <div className="modal-body mini-scroll">
                <div className="quick-stats">
                  ...
                  {(typeof selectedPlayer.assists === 'number' || (selectedPlayer.assists != null && selectedPlayer.assists !== '' && selectedPlayer.assists !== '-')) ? (
                ...
                {selectedPlayer.careerStations?.length > 0 && (
                  <div className="modal-section-box">
                    <h4>Career by Club / Team</h4>
                      ...
                      <table className="player-stats-table">
                        <thead>
                          <tr>
                            <th>Team</th>
                            <th>Period</th>
                            <th>{selectedPlayer.primaryStatLabel || 'Goals'}</th>
                            <th>{selectedPlayer.secondaryStatLabel || 'Assists'}</th>
                            <th>Apps</th>
                          </tr>
                        </thead>
                        <tbody>
                          {selectedPlayer.careerStations.map((s, i) => (
                            <tr key={i}>
                              <td>{s.team}</td>
                              <td>{s.period}</td>
                              <td>{typeof s.goals === 'number' ? s.goals.toLocaleString() : (s.goals ?? '-')}</td>
                              <td>{typeof s.assists === 'number' ? s.assists.toLocaleString() : (s.assists ?? '-')}</td>
                              <td>{typeof s.apps === 'number' ? s.apps.toLocaleString() : (s.apps ?? '-')}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {selectedPlayer.statsByCompetition && Object.keys(selectedPlayer.statsByCompetition).length > 0 && (
                  <div className="modal-section-box">
                    <h4>Stats by Competition</h4>
                    <div className="player-stats-table-wrap">
                      <table className="player-stats-table">
                        <thead>
                          <tr>
                            ...
                            <th>{selectedPlayer.primaryStatLabel || 'Goals'}</th>
                            <th>{selectedPlayer.secondaryStatLabel || 'Assists'}</th>
                            {Object.values(selectedPlayer.statsByCompetition).some(v => v.apps != null) ? <th>Apps</th> : null}
                          </tr>
                        </thead>
                        <tbody>
                          {Object.entries(selectedPlayer.statsByCompetition).map(([comp, stats], i) => (
                            <tr key={i}>
                              <td>{comp}</td>
                              <td>{typeof stats.goals === 'number' ? stats.goals.toLocaleString() : (stats.goals ?? '-')}</td>
                              <td>{typeof stats.assists === 'number' ? stats.assists.toLocaleString() : (stats.assists ?? '-')}</td>
                              {Object.values(selectedPlayer.statsByCompetition).some(v => v.apps != null) ? <td>{typeof stats.apps === 'number' ? stats.apps.toLocaleString() : (stats.apps ?? '-')}</td> : null}
                            </tr>
                          ))}
                        </tbody>
                      </table>
        ...
        {selectedMatchStatus && (() => {
          ...
          if (selectedSport === 'cricket' && hasStarted) tabs.push({ id: 'scorecard', label: 'Scorecard' });
          ...
          return (
            <div className="modal-overlay" onClick={() => { setSelectedMatchStatus(null); setMatchDetailTab('summary'); }}>
              <div className={`modal-content animate-in match-detail-modal ${selectedSport === 'cricket' ? 'match-detail-modal--cricket' : ''}`} onClick={e => e.stopPropagation()}>
                <button className="close-modal" onClick={() => { setSelectedMatchStatus(null); setMatchDetailTab('summary'); }}>&times;</button>
                <div className={`modal-hero match-hero-detail ${selectedSport === 'cricket' ? 'match-hero-detail--cricket' : ''}`}>
                  ...
                  {selectedSport === 'cricket' && hasStarted ? (
                    <>
                      <div className="cricket-score-line">
                        {selectedMatchStatus.status && <div className="cricket-result">{selectedMatchStatus.status}</div>}
                        <div className="cricket-score-main">
                          {cleanMainScore(selectedMatchStatus.homeScore)} – {cleanMainScore(selectedMatchStatus.awayScore)}
                        </div>
                        <div className="cricket-date-pill">
                          <span className="material-icons-round" style={{ fontSize: 14 }}>schedule</span>
                          {selectedMatchStatus.time}
                        </div>
                      </div>
                      {periodScores.length >= 2 && (
                        <div className="cricket-innings-bars">
                          {[...periodScores].sort((a, b) => (a.isHome ? -1 : 1)).map((ps, i) => (
                            <div key={i} className="cricket-team-bar">
                              <span className="cricket-team-bar-name">{ps.team}</span>
                              <span className="cricket-team-bar-score">{ps.totalScore}</span>
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  {/* Period/Quarter/Inning score strip (non-cricket only) */}
                  {selectedSport !== 'cricket' && hasStarted && periodScores.length >= 2 && periodScores[0].linescores?.length > 0 && (
                ...
                {(d.venue || d.attendance || d.weather) && (
                  <div className={`match-info-bar ${selectedSport === 'cricket' ? 'match-info-bar--cricket' : ''}`}>
                ...
                <div className="modal-body mini-scroll">
                  ...
                  {activeTab === 'summary' && (
                    <div className="match-detail-section">
                      {/* Cricket: Cricbuzz-style scorecard */}
                      {selectedSport === 'cricket' && hasStarted && (
                        <div className="cricket-scorecard">
                          {periodScores.length >= 2 && [...periodScores].sort((a, b) => (a.isHome ? -1 : 1)).map((ps, idx) => (
                            <div key={idx} className="cricket-innings-block">
                              <div className="cricket-innings-team-bar">
                                <span className="cricket-innings-team-name">{ps.team}</span>
                                <span className="cricket-innings-team-score">{ps.totalScore}</span>
                              </div>
                              {boxTeams[idx]?.statistics && (() => {
                                const battingStat = boxTeams[idx].statistics.find(s => /batting|runs|r\b/i.test(s.name || '') && s.athletes?.length);
                                const bowlingStat = boxTeams[(idx + 1) % 2]?.statistics?.find(s => /bowling|wicket|w\b|overs/i.test(s.name || '') && s.athletes?.length);
                                const battingRows = battingStat?.athletes || [];
                                const bowlingRows = bowlingStat?.athletes || [];
                                return (battingRows.length > 0 || bowlingRows.length > 0) ? (
                                  <>
                                    {battingRows.length > 0 && (
                                      <div className="cricket-table-wrap">
                                        <table className="cricket-scorecard-table cricket-batting-table">
                                          <thead>
                                            <tr>
                                              <th>Batter</th>
                                              <th>R</th>
                                          ...
                                          <tbody>
                                            {battingRows.map((a, i) => {
                                              const stat = a.statistics?.[0] || a;
                                              const r = stat.runs ?? stat.R ?? stat.displayValue;
                                              const b = stat.balls ?? stat.B;
                                              const fours = stat.fours ?? stat['4s'];
                                              const sixes = stat.sixes ?? stat['6s'];
                                              const sr = stat.strikeRate ?? stat.SR;
                                              return (
                                                <tr key={i}>
                                                  <td className="cricket-name-cell">{a.displayName || a.athlete?.displayName || '—'}</td>
                                                  <td>{r != null ? r : '—'}</td>
                                                  <td>{b != null ? b : '—'}</td>
                                                  <td>{fours != null ? fours : '—'}</td>
                                                  <td>{sixes != null ? sixes : '—'}</td>
                                                  <td>{sr != null ? sr : '—'}</td>
                                                </tr>
                                              );
                                            })}
                                    ...
                                    {bowlingRows.length > 0 && (
                                      <div className="cricket-table-wrap">
                                        <table className="cricket-scorecard-table cricket-bowling-table">
                                          <thead>
                                            <tr>
                                              <th>Bowler</th>
                                              <th>O</th>
                                          ...
                                          <tbody>
                                            {bowlingRows.map((a, i) => {
                                              const stat = a.statistics?.[0] || a;
                                              const o = stat.overs ?? stat.O;
                                              const m = stat.maidens ?? stat.M;
                                              const r = stat.runs ?? stat.R;
                                              const w = stat.wickets ?? stat.W;
                                              const eco = stat.economy ?? stat.ECO;
                                              return (
                                                <tr key={i}>
                                                  <td className="cricket-name-cell">{a.displayName || a.athlete?.displayName || '—'}</td>
                                                  <td>{o != null ? o : '—'}</td>
                      ...
                      {selectedSport === 'cricket' && hasStarted && d.manOfTheMatch?.name && (
                        <div className="cricket-mom-block">
                          <h4 className="cricket-mom-title">
                            <span className="material-icons-round" style={{ fontSize: 18, marginRight: 6 }}>emoji_events</span>
                            Man of the Match
                          </h4>
                          <div className="cricket-mom-card">
                            <span className="cricket-mom-name">{d.manOfTheMatch.name}</span>
                            {d.manOfTheMatch.team && <span className="cricket-mom-team">{d.manOfTheMatch.team}</span>}
                            {d.manOfTheMatch.value && <span className="cricket-mom-value">{d.manOfTheMatch.value}</span>}
                          </div>
                        </div>
                      )}
                      ...
                      {/* Boxscore stats summary - top 5 (skip for cricket; we show scorecard above) */}
                      {hasStarted && selectedSport !== 'cricket' && boxTeams.length >= 2 && boxTeams[0].statistics?.length > 0 && (
                      ...
                      <div className="updates-list">
                        {keyEvents.length > 0 ? keyEvents.map((ev, i) => (
                          <div key={i} className="update-item animate-in" style={{ animationDelay: `${i * 0.04}s` }}>
                            <div className="update-time-tag">{ev.clock?.displayValue || ev.shortText || (ev.type?.text || '').replace(/_/g, ' ')}</div>
                            <div className="update-icon">
                              <span className="material-icons-round">
                                {(() => {
                                  ...
                                  if (selectedSport === 'cricket') {
                                    if (t.includes('wicket') || txt.includes('wicket')) return 'sports_cricket';
                                    if (t.includes('run') || txt.includes(' runs') || txt.includes('50') || txt.includes('100')) return mCfg.eventIcons.goal;
                                    if (t.includes('innings') || t.includes('session')) return 'schedule';
                            ...
                            <div className="update-content">
                              <div className="update-type">{ev.type?.text ? ev.type.text.replace(/_/g, ' ') : (selectedSport === 'cricket' ? 'Update' : '')}</div>
                              <div className="update-text">{ev.text}</div>
                              {ev.participants?.map((p, pi) => (
                                <div key={p.athlete?.id || pi} className="update-player">{p.athlete?.displayName}</div>
                              ))}
                            </div>
                          </div>
                  ...
                  {/* ========== SCORECARD TAB (Cricket) ========== */}
                  {activeTab === 'scorecard' && selectedSport === 'cricket' && hasStarted && (
                    <div className="match-detail-section cricket-scorecard-tab">
                      <h4 className="scorecard-tab-title">Scorecard</h4>
                      {(() => {
                        const sc = d.cricketScorecard;
                        const innings = sc?.innings || (periodScores.length >= 2 && boxTeams.length >= 2 ? [...periodScores].sort((a, b) => (a.isHome ? -1 : 1)).map((ps, idx) => {
                          const battingStat = boxTeams[idx]?.statistics?.find(s => /batting|runs|r\b/i.test(s.name || '') && s.athletes?.length);
                          const bowlingStat = boxTeams[(idx + 1) % 2]?.statistics?.find(s => /bowling|wicket|w\b|overs/i.test(s.name || '') && s.athletes?.length);
                          const batting = (battingStat?.athletes || []).map(a => {
                            const stat = a.statistics?.[0] || a;
                            return { name: a.displayName || a.athlete?.displayName || '—', runs: stat.runs ?? stat.R ?? stat.displayValue, balls: stat.balls ?? stat.B, fours: stat.fours ?? stat['4s'], sixes: stat.sixes ?? stat['6s'], sr: stat.strikeRate ?? stat.SR, dismissal: a.dismissal?.text || (a.notOut ? 'not out' : '—') };
                          });
                          const bowling = (bowlingStat?.athletes || []).map(a => {
                            const stat = a.statistics?.[0] || a;
                            return { name: a.displayName || a.athlete?.displayName || '—', o: stat.overs ?? stat.O, m: stat.maidens ?? stat.M, r: stat.runs ?? stat.R, w: stat.wickets ?? stat.W, nb: stat.noBalls ?? stat.NB, wd: stat.wides ?? stat.WD, eco: stat.economy ?? stat.ECO };
                          });
                          return { team: ps.team, totalScore: ps.totalScore, overs: ps.linescores?.[0], runRate: null, batting, bowling, extras: null, didNotBat: [], fallOfWickets: null, powerplays: null, partnerships: null };
                        }) : []);
                        return innings.length === 0 ? (
                          <p className="scorecard-no-data">Scorecard data will appear here when available for this match.</p>
                        ) : (
                          innings.map((inn, i) => (
                            <div key={i} className="scorecard-innings-block">
                              <div className="scorecard-team-header">
                                <span className="scorecard-team-name">{inn.team}</span>
                                <span className="scorecard-team-score">{inn.totalScore}{inn.overs != null ? ` (${inn.overs} Ov)` : ''}{inn.runRate != null ? `, RR: ${inn.runRate}` : ''}</span>
                              </div>
                              {inn.batting?.length > 0 && (
                                <>
                                  <div className="scorecard-section-label">Batting</div>
                                  <div className="scorecard-table-wrap">
                                    <table className="scorecard-table scorecard-batting">
                                      <thead>
                                        <tr>
                                          <th>Batter</th>
                                          <th>R</th>
                                          ...
                                          <th>6s</th>
                                          <th>SR</th>
                                          {inn.batting.some(b => b.dismissal) && <th>Dismissal</th>}
                                        </tr>
                                      </thead>
                                      <tbody>
                                        {inn.batting.map((b, j) => (
                                          <tr key={j}>
                                            <td className="scorecard-name">{b.name}</td>
                                            <td>{b.runs != null ? b.runs : '—'}</td>
                                            <td>{b.balls != null ? b.balls : '—'}</td>
                                            <td>{b.fours != null ? b.fours : '—'}</td>
                                            <td>{b.sixes != null ? b.sixes : '—'}</td>
                                            <td>{b.sr != null ? b.sr : '—'}</td>
                                            {inn.batting.some(x => x.dismissal) && <td className="scorecard-dismissal">{b.dismissal || '—'}</td>}
                                          </tr>
                                        ))}
                                      </tbody>
                                    </table>
                                  </div>
                                  {(inn.extras || inn.totalScore) && (
                                    <div className="scorecard-extras-total">
                                      {inn.extras && <span>Extras: {inn.extras}</span>}
                                      <span>Total: {inn.totalScore}{inn.overs ? ` (${inn.overs} Overs${inn.runRate ? `, RR: ${inn.runRate}` : ''})` : ''}</span>
                                    </div>
                                  )}
                                  {Array.isArray(inn.didNotBat) && inn.didNotBat.length > 0 && (
                                    <div className="scorecard-dnb">Did not bat: {inn.didNotBat.join(', ')}</div>
                                  )}
                                </>
                              )}
                              {inn.bowling?.length > 0 && (
                                <>
                                  ...
                                  <div className="scorecard-table-wrap">
                                    <table className="scorecard-table scorecard-bowling">
                                      <thead>
                                        <tr>
                                          <th>Bowler</th>
                                          <th>O</th>
                                      ...
                                      <tbody>
                                        {inn.bowling.map((b, j) => (
                                          <tr key={j}>
                                            ...
                                            <td>{b.r != null ? b.r : '—'}</td>
                                            <td>{b.w != null ? b.w : '—'}</td>
                                            <td>{b.nb != null ? b.nb : '—'}</td>
                                            <td>{b.wd != null ? b.wd : '—'}</td>
                                            <td>{b.eco != null ? b.eco : '—'}</td>
                                          </tr>
                                        ))}
                                      </tbody>
                                    </table>
                              ...
                              {inn.fallOfWickets?.length > 0 && (
                                <>
                                  <div className="scorecard-section-label">Fall of Wickets</div>
                                  <div className="scorecard-table-wrap">
                                    <table className="scorecard-table scorecard-fow">
                                      <thead>
                                        <tr><th>Batsman</th><th>Score</th><th>Over</th></tr>
                                      </thead>
                                      <tbody>
                                        {inn.fallOfWickets.map((f, j) => (
                                          <tr key={j}>
                                            <td className="scorecard-name">{f.batsman}</td>
                                            <td>{f.score}</td>
                                            <td>{f.over}</td>
                                          </tr>
                                        ))}
                              ...
                              {inn.powerplays?.length > 0 && (
                                <>
                                  <div className="scorecard-section-label">Powerplays</div>
                                  <div className="scorecard-table-wrap">
                                    <table className="scorecard-table scorecard-powerplays">
                                      <thead>
                                        <tr><th>Type</th><th>Overs</th><th>Runs</th></tr>
                                      </thead>
                                      <tbody>
                                        {inn.powerplays.map((p, j) => (
                                          <tr key={j}>
                                            <td>{p.type}</td>
                                            <td>{p.overs}</td>
                                            <td>{p.runs}</td>
                              ...
                              {inn.partnerships?.length > 0 && (
                                <>
                                  <div className="scorecard-section-label">Partnerships</div>
                                  <div className="scorecard-partnerships">
                                    {inn.partnerships.map((p, j) => (
                                      <div key={j} className="scorecard-partnership-row">{typeof p === 'object' && p.text ? p.text : String(p)}</div>
                                    ))}
                                  </div>
                                </>
                              )}
