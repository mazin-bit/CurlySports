...
const SECTIONS: [string, [string, string, string, string?][]][] = [
  ['navSection.app', [
    ['home', 'nav.home', 'home'],
    ['live', 'nav.liveScores', 'live'],
    ['leagues', 'nav.leagues', 'trophy'],
    ['teams', 'nav.teams', 'user'],
    ['players', 'nav.players', 'user'],
    ['news', 'nav.news', 'news'],
    ...
    <>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 101, background: 'var(--bg-2)', borderTop: '2.5px solid var(--ink)', borderRadius: '20px 20px 0 0', boxShadow: '0 -8px 40px rgba(12,10,29,0.18)', maxHeight: '88%', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'cs-mnSlideUp 0.28s var(--ease-pop)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 18px 10px', borderBottom: '1.5px solid var(--border-2)' }}>
          <span style={{ fontFamily: 'var(--display)', fontWeight: 800, fontSize: 16, color: 'var(--ink)' }}>{t('menu.title')}</span>
          <button onClick={onClose} aria-label={t('menu.close')} style={{ width: 30, height: 30, display: 'grid', placeItems: 'center', background: 'var(--surface-3)', border: '1.5px solid var(--border-2)', borderRadius: 8, cursor: 'pointer', color: 'var(--text-dim)' }}>
              ...
              {items.map(([key, nameKey, icon, badgeKey]) => {
                ...
                return (
                  <button key={key} onClick={() => onNavigate(key)} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: 10, borderRadius: 10, marginBottom: 2, cursor: 'pointer', textAlign: 'start', border: `1.5px solid ${isActive ? 'var(--ink)' : 'transparent'}`, background: isActive ? 'var(--accent)' : 'transparent', boxShadow: isActive ? 'var(--shadow-sm)' : 'none', color: isActive ? 'var(--ink)' : 'var(--text-dim)', fontWeight: isActive ? 700 : 600, fontSize: 13.5 }}>
                    ...
                    <span style={{ flex: 1 }}>{t(nameKey)}</span>
                    {badgeKey && <Badge tone={isActive ? 'ink' : 'mute'}>{t(badgeKey)}</Badge>}
                  </button>
                );
              })}
            </div>
        ...
        <div style={{ flexShrink: 0, borderTop: '2px solid var(--border-2)', padding: '12px 14px 18px', background: 'var(--bg-2)' }}>
          <button onClick={() => onNavigate('profile')} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 12px', background: 'var(--surface)', border: '2px solid var(--ink)', borderRadius: 12, boxShadow: 'var(--shadow-sm)', cursor: 'pointer', textAlign: 'start' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-mute)' }}>{t('profile.viewProfile')}</div>
            </div>
