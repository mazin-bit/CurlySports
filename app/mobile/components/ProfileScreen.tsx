...
function Toggle({ on, onClick }: { on: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ width: 46, height: 27, borderRadius: 999, border: '2px solid var(--ink)', background: on ? 'var(--accent)' : 'var(--surface-3)', position: 'relative', cursor: 'pointer', flexShrink: 0, transition: 'background .15s', boxShadow: 'var(--shadow-sm)' }}>
...
function Row({ icon, color, title, sub, children, last }: { icon: string; color: string; title: string; sub?: string; children?: React.ReactNode; last?: boolean }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: last ? 'none' : '1px solid var(--border-3)' }}>
      <span style={{ width: 34, height: 34, flexShrink: 0, borderRadius: 9, background: color, border: '2px solid var(--ink)', display: 'grid', placeItems: 'center', color: 'var(--ink)' }}><Icon name={icon} size={16} /></span>
...
export default function ProfileScreen({ fav, onSearch, onBell, unread }: ProfileProps) {
  ...
  const team = fav ?? { code: 'mun', name: 'Man United' };
  const D = DATA;

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      <Topbar title="You" subtitle="Profile & settings" logoSrc={D.mascot} onSearch={onSearch} onBell={onBell} hasNotification={unread > 0} />
      <div className="cs-scroll" style={{ flex: 1, overflow: 'auto', padding: '14px 14px 96px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Card style={{ background: 'var(--ink)', borderColor: 'var(--ink)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 60, height: 60, borderRadius: 16, background: 'var(--lime)', border: '2px solid var(--paper)', display: 'grid', placeItems: 'center', overflow: 'hidden', transform: 'rotate(-6deg)', flexShrink: 0 }}>
              <img src={D.mascot} alt="" style={{ width: '84%', height: '84%', objectFit: 'contain', transform: 'rotate(6deg)' }} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--display)', fontWeight: 800, fontSize: 22, color: 'var(--paper)', letterSpacing: '-0.02em' }}>Mazin</div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'rgba(255,253,247,0.6)', marginTop: 2 }}>@mazin · mazin@curly.sports</div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginTop: 16, borderTop: '1px solid rgba(255,253,247,0.15)', paddingTop: 14 }}>
            {[['Debates', '48'], ['Following', '12'], ['Day streak', '31']].map(([l, v]) => (
        ...
        <button style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: 14, background: 'var(--surface)', border: '2px solid var(--border-2)', borderRadius: 12, fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--text-mute)', cursor: 'pointer' }}>
