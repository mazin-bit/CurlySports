...
export default function TeamsPage() {
  ...
  return (
    <AppShell active="teams" title="Teams" subtitle={`${activeSportConfig.label} clubs`}>
      <div className="stack">
        {leagues.length > 1 && (
        ...
        <div>
          <div className="sec-head">
            <div className="title">
              ...
              {activeSportConfig.icon} All <span className="accent">Teams</span>
            </div>
            <span className={styles.count}>{isLoading ? "Loading…" : `${teams.length} clubs`}</span>
          </div>
          ...
          {isLoading ? (
            ...
            <div style={{ textAlign: "center", padding: "48px 20px", color: "var(--text-mute)", fontSize: 14 }}>
              ...
              No teams data available for {activeSportConfig.label}.
            </div>
          ) : (
            <div className={styles.teamsGrid}>
